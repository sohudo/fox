...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GP_REGISTERCHECKRESULT:    //注册检查结果
		{
			return OnDBPRegisterCheckResult(dwContextID,pData,wDataSize);
		}
	...
	case DBO_GP_USER_INSURE_LOGON_SUCCESS:			//验证成功
		{
			return OnDBPUserInsureIogonSuccess(dwContextID,pData,wDataSize);
		}
	case DBO_GP_USER_INSURE_LOGON_FAILURE:			//验证失败
		{
			return OnDBPUserInsureIogonFailure(dwContextID,pData,wDataSize);
		}
	...
	case DBO_GP_BANK_DETAIL_RESULT:
		{
...
bool CAttemperEngineSink::OnTCPNetworkMainPCLogon(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GP_REGISTER_CHECK:  //注册检查
		{
			return OnTCPNetWorkSubPCRegisterCheck(pData,wDataSize,dwSocketID);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_GP_USER_RECHARGE:		//绑定机器
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_UserRecharge));
			if (wDataSize!=sizeof(CMD_GP_UserRecharge)) return false;

			//处理消息
			CMD_GP_UserRecharge * pUserRecharge=(CMD_GP_UserRecharge *)pData;
			pUserRecharge->szPassword[CountArray(pUserRecharge->szPassword)-1]=0;

			//变量定义
			DBR_GP_UserRecharge UserRecharge;
			ZeroMemory(&UserRecharge,sizeof(UserRecharge));

			//构造数据
			lstrcpyn(UserRecharge.szID,pUserRecharge->szID,CountArray(UserRecharge.szID));
			lstrcpyn(UserRecharge.szPassword,pUserRecharge->szPassword,CountArray(UserRecharge.szPassword));
			lstrcpyn(UserRecharge.szCard,pUserRecharge->szCard,CountArray(UserRecharge.szCard));

			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_USER_RECHARGE,dwSocketID,&UserRecharge,sizeof(UserRecharge));
	...
	case SUB_GP_USER_SAVE_SCORE:	//存入欢乐豆
		{
			//效验参数
	...
	case SUB_GP_USER_TAKE_SCORE:	//取出欢乐豆
		{
			//效验参数
	...
	case SUB_GP_USER_TRANSFER_SCORE://转账欢乐豆
		{
			//效验参数
	...
	case SUB_GP_CHANGE_INSURE_PASSWORD:	
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_ModifyInsurePass));
			if (wDataSize!=sizeof(CMD_GP_ModifyInsurePass)) return false;
	...
	case SUB_GP_USER_INSURE_LOGON:
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_UserInsureLogon));
			if (wDataSize!=sizeof(CMD_GP_UserInsureLogon)) return false;

			//处理消息
			CMD_GP_UserInsureLogon * pInsureLogon=(CMD_GP_UserInsureLogon *)pData;

			//变量定义
			DBR_GP_UserInsureLogon UserInsureLogon;
			ZeroMemory(&UserInsureLogon,sizeof(UserInsureLogon));

			//构造数据
			UserInsureLogon.dwUserID=pInsureLogon->dwUserID;
			UserInsureLogon.dwClientAddr=(m_pBindParameter+LOWORD(dwSocketID))->dwClientAddr;
			lstrcpyn(UserInsureLogon.szPassword,pInsureLogon->szPassword,CountArray(UserInsureLogon.szPassword));
			lstrcpyn(UserInsureLogon.szMachineID,pInsureLogon->szMachineID,CountArray(UserInsureLogon.szMachineID));

			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_USER_INSURE_LOGON,dwSocketID,&UserInsureLogon,sizeof(UserInsureLogon));

			return true;
		}

	case SUB_GP_QUERY_BANK_DETAIL:	//查询银行明细
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_GP_QueryBankDetail));
			if (wDataSize!=sizeof(CMD_GP_QueryBankDetail)) return false;

			//处理消息
			CMD_GP_QueryBankDetail * pQueryBankDetail=(CMD_GP_QueryBankDetail *)pData;

			//变量定义
			DBR_GP_QueryBankDetail QueryIndividual;
			ZeroMemory(&QueryIndividual,sizeof(QueryIndividual));
			...
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_QUERY_BANK_DETAIL,dwSocketID,&QueryIndividual,sizeof(QueryIndividual));
...
//账号检测
bool CAttemperEngineSink::OnTCPNetWorkSubPCRegisterCheck(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
	ASSERT(wDataSize>=sizeof(CMD_GP_RegisterCheck));
	if (wDataSize<sizeof(CMD_GP_RegisterCheck))
	{
		if (wDataSize<sizeof(CMD_GP_RegisterCheck)-sizeof(BYTE))
			return false;
	}

	//处理消息
	CMD_GP_RegisterCheck* pRegisterCheck = (CMD_GP_RegisterCheck*)pData;
	pRegisterCheck->szCheckInfo[CountArray(pRegisterCheck->szCheckInfo)-1]=0;

	//构建数据
	DBR_GP_RegisterCheck RegisterCheck;
	ZeroMemory(&RegisterCheck,sizeof(RegisterCheck));
	lstrcpyn(RegisterCheck.szCheckInfo,pRegisterCheck->szCheckInfo,CountArray(pRegisterCheck->szCheckInfo));
	RegisterCheck.cbCheckType = pRegisterCheck->cbCheckType;
	RegisterCheck.cbResult    = pRegisterCheck->cbResult;

	//投递请求
	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_REGISTER_CHECK,dwSocketID,&RegisterCheck,sizeof(RegisterCheck));
...
//额外信息
bool CAttemperEngineSink::OnDBPCNeedExtern(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	...
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_LOGON,SUB_GP_NEED_EXTERN);
...
//检查结果
bool CAttemperEngineSink::OnDBPRegisterCheckResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	ASSERT(LOWORD(dwContextID)<m_pInitParameter->m_wMaxConnect);
	if ((m_pBindParameter+LOWORD(dwContextID))->dwSocketID!=dwContextID) return true;

	DBO_GP_RegisterCheckResult* pCheckResult = (DBO_GP_RegisterCheckResult*)pData;

	CMD_GP_CheckResult CheckResult;
	ZeroMemory(&CheckResult,sizeof(CMD_GP_CheckResult));
	CheckResult.cbCheckType = pCheckResult->cbCheckType;
	CheckResult.cbResult = pCheckResult->cbResut;
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_LOGON,SUB_GP_REGISTER_CHECKRESULT,&CheckResult,sizeof(CMD_GP_CheckResult));
...
{
	//效验参数
	ASSERT(wDataSize%sizeof(DBO_GP_BankDetailResult)==0);
	if (wDataSize%sizeof(DBO_GP_BankDetailResult)!=0) return false;

	//变量定义
	WORD wItemCount=wDataSize/sizeof(DBO_GP_BankDetailResult);
	DBO_GP_BankDetailResult * pBankDetailResult=(DBO_GP_BankDetailResult *)pData;
	...
	{
		//发送数据
		if ((wSendSize+sizeof(DBO_GP_BankDetailResult))>sizeof(cbDataBuffer))
		{
			m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER_SERVICE,SUB_GP_BANK_DETAIL_RESULT,cbDataBuffer,wSendSize);
			wSendSize=0;
		}

		//拷贝数据
		CMD_GP_BankDetailResult* pDetailResult=(CMD_GP_BankDetailResult *)(cbDataBuffer+wSendSize);
		...
		wSendSize+=sizeof(CMD_GP_BankDetailResult);
	}

	//发送剩余
	if (wSendSize>0) m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER_SERVICE,SUB_GP_BANK_DETAIL_RESULT,cbDataBuffer,wSendSize);

	return true;
}

//验证成功
bool CAttemperEngineSink::OnDBPUserInsureIogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//变量定义
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER_SERVICE,SUB_GP_USER_INSURE_LOGON_SUCCESS,NULL,0);

	return true;
}
//验证失败
bool CAttemperEngineSink::OnDBPUserInsureIogonFailure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{

	//变量定义
	CMD_GP_UserInsureLogonFailure UserInsureLogonFailure;
	ZeroMemory(&UserInsureLogonFailure,sizeof(UserInsureLogonFailure));
	DBR_GP_UserInsureLogonFailure * pUserInsureLogonFailure=(DBR_GP_UserInsureLogonFailure *)pData;

	//构造数据
	UserInsureLogonFailure.lResultCode=pUserInsureLogonFailure->lResultCode;
	lstrcpyn(UserInsureLogonFailure.szDescribeString,pUserInsureLogonFailure->szDescribeString,CountArray(UserInsureLogonFailure.szDescribeString));

	//发送数据
	WORD wStringSize=CountStringBuffer(UserInsureLogonFailure.szDescribeString);
	WORD wSendSize=sizeof(UserInsureLogonFailure)-sizeof(UserInsureLogonFailure.szDescribeString)+wStringSize;
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER_SERVICE,SUB_GP_USER_INSURE_LOGON_FAILURE,&UserInsureLogonFailure,wSendSize);
