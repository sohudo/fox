...
#define DBR_GP_REGISTER_CHECK       4                                   //注册检查
...
#define DBR_GP_QUERY_BANK_DETAIL	15									//交易记录
#define DBR_GP_USER_RECHARGE		16									//修改机器
...
//银行命令
#define DBR_GP_USER_SAVE_SCORE		30									//存入欢乐豆
#define DBR_GP_USER_TAKE_SCORE		31									//提取欢乐豆
#define DBR_GP_USER_TRANSFER_SCORE	32									//转账欢乐豆
#define DBR_GP_USER_INSURE_LOGON	34							        //银行登录
...
//注册检测
struct DBR_GP_RegisterCheck
{
	TCHAR                           szCheckInfo[LEN_NICKNAME];          //检查信息
	BYTE                            cbCheckType;                        //检查类型
	BYTE                            cbResult;                           //检查结果                         
};
//用户充值
struct DBR_GP_UserRecharge
{
	TCHAR							szID[LEN_ACCOUNTS];			//用户密码
	TCHAR							szPassword[LEN_PASSWORD];			//用户密码
	TCHAR							szCard[LEN_PASSWORD];		       //用户卡号
};
//修改机器
...
struct DBR_GP_UserInsureLogonFailure
{
...
//查询资料
struct DBR_GP_QueryBankDetail
{
	DWORD							dwUserID;							//用户 I D
	BYTE							cbTransferIn;
};

struct DBO_GP_BankDetailResult
{
	BYTE							cbState;
...
#define DBO_GP_NEED_EXTERN			103									//创建角色
#define DBO_GP_REGISTERCHECKRESULT  104                                 //检查结果
...
#define DBO_GP_BANK_DETAIL_RESULT	124									//交易记录
#define DBO_GP_USER_INSURE_LOGON_SUCCESS   126
#define DBO_GP_USER_INSURE_LOGON_FAILURE   127
...
{
	DWORD							dwUserID;							//用户 I D
	SCORE							lSourceScore;						//原来欢乐豆
	SCORE							lSourceInsure;						//原来欢乐豆
	SCORE							lInsureRevenue;						//银行税收
	SCORE							lVariationScore;					//欢乐豆变化
...
//验证成功
struct DBR_GP_CheckSuccess
{
	TCHAR							szDescribeString[128];				//描述消息
};
//验证失败
struct DBR_GP_CheckFailure
{
	LONG							lResultCode;						//错误代码
	TCHAR							szDescribeString[128];				//描述消息
};
...
//注册检查结果 
struct DBO_GP_RegisterCheckResult
{
	BYTE                            cbCheckType;                       //检查类型
	BYTE                            cbResut;                           //结构
};
