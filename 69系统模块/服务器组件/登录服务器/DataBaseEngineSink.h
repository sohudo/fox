...
protected:
	...
	//欢乐豆数据库
protected:
	CDataBaseAide					m_TreasureDBAide;					//欢乐豆数据库
	CDataBaseHelper					m_TreasureDBModule;					//欢乐豆数据库
...
protected:
	...
	//注册检测
	bool OnRequsteRegisterCheck(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//手机登录
protected:
	//I D 登录
...
protected:
	//用户充值
	bool OnRequestUserRecharge(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//修改机器
...
protected:
	//登陆查询
	bool OnRequestUserInsureLogon(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//查询资料
	...
	//查询明细
	bool OnRequestQueryBankDetail(DWORD dwContextID, VOID * pData, WORD wDataSize);
