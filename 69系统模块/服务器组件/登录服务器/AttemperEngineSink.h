...
protected:
	...
	//帐号验证
	bool OnTCPNetWorkSubPCRegisterCheck(VOID * pData, WORD wDataSize, DWORD dwSocketID);
...
protected:
	//额外信息
	bool OnDBPCNeedExtern(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//登录成功
	...
	//验证成功
	bool OnDBPUserInsureIogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//验证失败
	bool OnDBPUserInsureIogonFailure(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//用户信息
	...
	//检查结果
	bool OnDBPRegisterCheckResult(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//明细结果
	bool OnDBPCBankDetailResult(DWORD dwContextID, VOID * pData, WORD wDataSize);
