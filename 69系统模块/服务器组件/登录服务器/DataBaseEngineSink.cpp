...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_REGISTER_CHECK: //注册检查
		{
			return OnRequsteRegisterCheck(dwContextID,pData,wDataSize);
		}
	case DBR_GP_USER_RECHARGE:			//用户充值
		{
			return OnRequestUserRecharge(dwContextID,pData,wDataSize);
		}
	...
	case DBR_GP_QUERY_BANK_DETAIL:
		{
			return OnRequestQueryBankDetail(dwContextID, pData, wDataSize);
		}
	case DBR_GP_USER_INSURE_LOGON:
		{
			return OnRequestUserInsureLogon(dwContextID, pData, wDataSize);
		}
...
//注册检测
bool CDataBaseEngineSink::OnRequsteRegisterCheck(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize==sizeof(DBR_GP_RegisterCheck));
		if (wDataSize!=sizeof(DBR_GP_RegisterCheck)) return false;

		DBR_GP_RegisterCheck* pRegisterCheck = (DBR_GP_RegisterCheck*)pData;

		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwInputType"),pRegisterCheck->cbCheckType);
		m_AccountsDBAide.AddParameter(TEXT("@strInputName"),pRegisterCheck->szCheckInfo);

		//执行查询
		LONG lResultCode=m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GP_ValidRegistName"),true);

		DBO_GP_RegisterCheckResult RegisterCheckResult;
		ZeroMemory(&RegisterCheckResult,sizeof(RegisterCheckResult));
		RegisterCheckResult.cbCheckType = pRegisterCheck->cbCheckType;
		RegisterCheckResult.cbResut = (BYTE)lResultCode;

		//发送结果
		WORD wDataSize=sizeof(DBO_GP_RegisterCheckResult);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_REGISTERCHECKRESULT,dwContextID,&RegisterCheckResult,wDataSize);
	}
	catch (IDataBaseException * pIException)
	{
		//错误信息
		...
		//错误处理
		OnLogonDisposeResult(dwContextID,DB_ERROR,TEXT("很抱歉，该操作当前无法执行，请您稍后重试或选择另一服务器登录！"),false);
...
//用户充值
bool CDataBaseEngineSink::OnRequestUserRecharge(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize==sizeof(DBR_GP_UserRecharge));
		if (wDataSize!=sizeof(DBR_GP_UserRecharge)) return false;

		//请求处理
		DBR_GP_UserRecharge * pUserRecharge=(DBR_GP_UserRecharge *)pData;

		//转化地址
		TCHAR szClientAddr[16]=TEXT("");
		//BYTE * pClientAddr=(BYTE *)&pModifyMachine->dwClientAddr;
		_sntprintf(szClientAddr,CountArray(szClientAddr),TEXT("0.0.0.0"));
        int ii=0;
		//构造参数
		m_AccountsDBAide.ResetParameter();
		m_AccountsDBAide.AddParameter(TEXT("@dwOperUserID"),ii);
		
		m_AccountsDBAide.AddParameter(TEXT("@strSerialID"),pUserRecharge->szCard);
		m_AccountsDBAide.AddParameter(TEXT("@strPassword"),pUserRecharge->szPassword);
		m_AccountsDBAide.AddParameter(TEXT("@strAccounts"),pUserRecharge->szID);
		m_AccountsDBAide.AddParameter(TEXT("@strClientIP"),szClientAddr);
		...
		m_AccountsDBAide.AddParameterOutput(TEXT("@strErrorDescribe"),szDescribe,sizeof(szDescribe),adParamOutput);
			...
			m_AccountsDBAide.ExecuteProcess(TEXT("NET_PW_FilledLivcard"),false);
...
bool CDataBaseEngineSink::OnRequestUserInsureLogon(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize==sizeof(DBR_GP_UserInsureLogon));
		if (wDataSize!=sizeof(DBR_GP_UserInsureLogon)) return false;

		//请求处理
		DBR_GP_UserInsureLogon * pUserInsureLogon=(DBR_GP_UserInsureLogon *)pData;

		//转化地址
		TCHAR szClientAddr[16]=TEXT("");
		BYTE * pClientAddr=(BYTE *)&pUserInsureLogon->dwClientAddr;
		_sntprintf(szClientAddr,CountArray(szClientAddr),TEXT("%d.%d.%d.%d"),pClientAddr[0],pClientAddr[1],pClientAddr[2],pClientAddr[3]);

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"),pUserInsureLogon->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@strPassword"),pUserInsureLogon->szPassword);
		m_TreasureDBAide.AddParameter(TEXT("@strClientIP"),szClientAddr);
		m_TreasureDBAide.AddParameter(TEXT("@strMachineID"),pUserInsureLogon->szMachineID);
		...
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_UserInsureLogon"),true)==DB_SUCCESS)
		{
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_USER_INSURE_LOGON_SUCCESS,dwContextID,NULL,0);
		}
		else
		{
			//变量定义
			DBR_GP_UserInsureLogonFailure UserInsureLogonFailure;
			...
			//构造变量
			UserInsureLogonFailure.lResultCode=m_TreasureDBModule->GetReturnValue();
			lstrcpyn(UserInsureLogonFailure.szDescribeString,CW2CT(DBVarValue.bstrVal),CountArray(UserInsureLogonFailure.szDescribeString));
			...
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_USER_INSURE_LOGON_FAILURE,dwContextID,&UserInsureLogonFailure,wSendSize);
		}
		return true;
	}
	catch (IDataBaseException * pIException)
...
//查询银行明细
bool CDataBaseEngineSink::OnRequestQueryBankDetail(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//效验参数
		ASSERT(wDataSize==sizeof(DBR_GP_QueryBankDetail));
		if (wDataSize!=sizeof(DBR_GP_QueryBankDetail)) return false;

		//请求处理
		DBR_GP_QueryBankDetail * pQueryBankDetail=(DBR_GP_QueryBankDetail *)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"),pQueryBankDetail->dwUserID);
		m_TreasureDBAide.AddParameter(TEXT("@cbTransferIn"),pQueryBankDetail->cbTransferIn);
		m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_QueryBankDetail"),true);

		//发送记录
		WORD wPacketSize=0;
		BYTE cbBuffer[MAX_ASYNCHRONISM_DATA];
		DBO_GP_BankDetailResult * pBankDetailResult=NULL;
		while (m_TreasureDBModule->IsRecordsetEnd()==false)
		{
			//发送信息
			if ((wPacketSize+sizeof(DBO_GP_BankDetailResult))>sizeof(cbBuffer))
			{
				m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_BANK_DETAIL_RESULT,dwContextID,cbBuffer,wPacketSize);
				wPacketSize=0;
			}

			//读取信息
			pBankDetailResult=(DBO_GP_BankDetailResult *)(cbBuffer+wPacketSize);
			SYSTEMTIME st;
			pBankDetailResult->cbState = 1;
			m_TreasureDBAide.GetValue_String(TEXT("NickName"), pBankDetailResult->szNickName, LEN_NICKNAME);
			pBankDetailResult->dwGameID = m_TreasureDBAide.GetValue_DWORD(TEXT("GameID"));
			pBankDetailResult->lSwapScore=m_TreasureDBAide.GetValue_LONGLONG(TEXT("SwapScore"));
			m_TreasureDBAide.GetValue_SystemTime(TEXT("CollectDate"), st);
			_sntprintf(pBankDetailResult->szDateTime, CountArray(pBankDetailResult->szDateTime), TEXT("%d-%02d-%02d %02d:%02d:%02d"), st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);

			//设置位移
			wPacketSize+=sizeof(DBO_GP_BankDetailResult);
		...
		if (wPacketSize>0)
		{
			pBankDetailResult=(DBO_GP_BankDetailResult *)(cbBuffer+wPacketSize-sizeof(DBO_GP_BankDetailResult));
			pBankDetailResult->cbState = 2;
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_BANK_DETAIL_RESULT,dwContextID,cbBuffer,wPacketSize);
		}
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient)
{
	if (bMobileClient==false)
	{
		if (dwErrorCode==DB_SUCCESS)
		{
			...
			////创建角色
			//if(LogonSuccess.szNickName[0]==0)
			//{
			//	m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_NEED_EXTERN,dwContextID,NULL,0);
			//	return;
			//}
