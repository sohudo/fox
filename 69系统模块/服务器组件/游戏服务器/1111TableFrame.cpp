	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if ((m_pGameServiceOption->lMinTableScore!=0L)&&(pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinTableScore))
			{
				...
				{
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("您的欢乐豆少于 ") SCORE_STRING TEXT("，不能继续游戏！"),m_pGameServiceOption->lMinTableScore);
				}
				else
				{
...
//校验机器人分数
BYTE CTableFrame::EfficayAndroidScoreMax(IServerUserItem * pIServerUserItem)
{
	//机器人的钱小于多少会直接退出 //不用于16人的游戏
	...
	if(m_wChairCount < 16)
	{
		...
		{
			//最少钱数
			if(pIServerUserItem->GetUserScore() < 10000)
			{
				return 1;
			}
			
			//坐不下去了
			if(pIServerUserItem->GetUserScore() < m_pGameServiceOption->lMinTableScore)return 1;

			//高级房间
			if(m_pGameServiceOption->lMinEnterScore >= 5000000)
			{
				srand(GetTickCount());
				SCORE lMaxScore=m_pGameServiceOption->lMinEnterScore*(rand()%340+60);
				INT uRand=rand()%100;
				if(uRand > 60)
				{
					if(pIServerUserItem->GetUserScore() > lMaxScore)
					{
						return 2;
					}else
					{
			...
			//中级房间
			if(m_pGameServiceOption->lMinEnterScore >= 1000000)
			{
				INT uRand=rand()%100;
				if(uRand > 60)
				{
					SCORE lMaxScore=m_pGameServiceOption->lMinEnterScore*(rand()%100+50);
					if(pIServerUserItem->GetUserScore() > lMaxScore)
					{
						return 2;
					}else
			...
			//普通房间
			if(m_pGameServiceOption->lMinEnterScore >= 100000)
			{
				INT uRand=rand()%100;
				if(uRand > 60)
				{
					SCORE lMaxScore=m_pGameServiceOption->lMinEnterScore*(rand()%800+100);
					if(pIServerUserItem->GetUserScore() > lMaxScore)
					{
						return 2;
					}
			...
			{
				SCORE lMaxScore=5000000+rand()%200000;
				if(pIServerUserItem->GetUserScore() > lMaxScore)
				{
					return 2;
				}
...
bool CTableFrame::WriteUserScore(WORD wChairID, tagScoreInfo & ScoreInfo, DWORD dwGameMemal, DWORD dwPlayGameTime)
{
	...
	if (pIServerUserItem->GetUserStatus() == US_NULL || pIServerUserItem->GetUserStatus() == US_OFFLINE || pIServerUserItem == NULL)return false;
	//写入积分
	if (pIServerUserItem!=NULL)
	{
		//变量定义
...
bool CTableFrame::EfficacyStartGame(WORD wReadyChairID)
{
	...
	for (WORD i=0;i<m_wChairCount;i++)
	{
		...
		{
			//机器人处理 防止准备卡死
			if((pITableUserItem->IsAndroidUser()==true) && (m_wChairCount==16))
			{
				if(pITableUserItem->GetUserStatus()==US_READY)
				{
					pITableUserItem->SetClientReady(true);
				}
			}
...
bool CTableFrame::EfficacyEnterTableScoreRule(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	if (((lMinTableScore!=0L)&&(lUserScore<lMinTableScore))||((lLessEnterScore!=0L)&&(lUserScore<lLessEnterScore)))
	{
		...
		TCHAR szDescribe[128]=TEXT("");
		if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_STRING TEXT(" 的欢乐豆，您的欢乐豆不够，不能加入！"),__max(lMinTableScore,lLessEnterScore));
