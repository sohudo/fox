...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_QUERY_TRANSFER_USER_INFO:		//查询用户
		...
		break;
	case DBR_GR_QUERY_BANK_DETAIL:		//银行记录
		{
			bSucceed = OnRequestQueryInsureRecord(dwContextID,pData,wDataSize,dwUserID);
		}
		break;
...
//查询银行
bool CDataBaseEngineSink::OnRequestQueryInsureRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		//效验参数
		ASSERT(wDataSize==sizeof(DBR_GR_QueryBankDetail));
		if (wDataSize!=sizeof(DBR_GR_QueryBankDetail)) return false;

		//请求处理
		DBR_GR_QueryBankDetail * pQueryBankDetail=(DBR_GR_QueryBankDetail *)pData;

		dwUserID=pQueryBankDetail->dwUserID;
		...
		DBO_GR_BankDetailResult * pBankDetailResult=NULL;

		while (m_TreasureDBModule->IsRecordsetEnd()==false)
		{
			//发送信息
			if ((wPacketSize+sizeof(DBO_GR_BankDetailResult))>sizeof(cbBuffer))
			{
				m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_BANK_DETAIL_RESULT,dwContextID,cbBuffer,wPacketSize);
				wPacketSize=0;
			}

			//读取信息
			pBankDetailResult=(DBO_GR_BankDetailResult *)(cbBuffer+wPacketSize);
			SYSTEMTIME st;
			...
			//设置位移
			wPacketSize+=sizeof(DBO_GR_BankDetailResult);
		...
		if (wPacketSize>0)
		{
			pBankDetailResult=(DBO_GR_BankDetailResult *)(cbBuffer+wPacketSize-sizeof(DBO_GR_BankDetailResult));
	...
	catch (IDataBaseException * pIException)
	{
		...
		//结果处理
		OnInsureDisposeResult(dwContextID,DB_ERROR,0L,TEXT("由于数据库操作异常，请您稍后重试！"),false,0);
