...
#define IDI_INTERLUDE_ANDROID       (IDI_MAIN_MODULE_START+12)			//机器人穿插房间
//////////////////////////////////////////////////////////////////////////////////
//时间定义 秒

#define TIME_LOAD_ANDROID_USER		180L								//加载机器
#define TIME_DISTRIBUTE_ANDROID		5L									//分配用户
#define TIME_INTERLUDE_ANDROID		60L								    //机器人重新分配时间
...
////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < MAX_TABLE; i++){
		DistributingRobot[i] = 0;//初始化每桌机器人数目
	}
	Active = 0;                            //主动
	Passive = 0;                           //被动
	Reciprocal = 0;                        //相互
...
//////////////////////////////////////////////////////////////////////////////
	...
	CTableFrame * pTableFrame = m_TableFrameArray[0];//获取桌子
	Table = m_pGameServiceOption->wTableCount;//获得房间桌子数
	Chair = m_pGameServiceAttrib->wChairCount;//获得房间椅子数
	StartMode = pTableFrame->GetStartMode();//获得开始模式
	if ((Chair < MAX_CHAIR) && (StartMode == START_MODE_TIME_CONTROL))//如果是时间控制模式
	{
		aa.Format(TEXT("时间控制非百人游戏启动随机分配"));
		CTraceService::TraceString(aa, TraceLevel_Info);
		for (int i = 0; i < Table; i++){
			Sleep(1);
			if (Chair == 1){
				DistributingRobot[i] = rand() % (Chair + 1);//初始化每桌机器人数目单人游戏
			}
			else{
				DistributingRobot[i] = rand() % Chair;//初始化每桌机器人数目
			}
		}
	}
	else
	if ((Chair < MAX_CHAIR) && (StartMode == START_MODE_ALL_READY)){
		aa.Format(TEXT("所有准备非百人游戏启动随机+1分配"));
		CTraceService::TraceString(aa, TraceLevel_Info);
		for (int i = 0; i < Table; i++){
			Sleep(1);
			if (Chair == 1){
				DistributingRobot[i] = rand() % (Chair + 1);//初始化每桌机器人数目
			}
			else{
				DistributingRobot[i] = (rand() % Chair) + 1;//初始化每桌机器人数目
			}
	...
	{
		aa.Format(TEXT("百人游戏和满人开始模式的只能启动固定分配"));
		CTraceService::TraceString(aa, TraceLevel_Info);
		for (int i = 0; i < Table; i++){
			DistributingRobot[i] = Chair;//初始化每桌机器人数目
		}
	}
	aa.Format(TEXT("游戏开始模式:0x%x"), pTableFrame->GetStartMode());
	CTraceService::TraceString(aa, TraceLevel_Info);

	aa.Format(TEXT("本房间桌子数:%d,每张桌子%d把椅子."), Table, Chair);
	CTraceService::TraceString(aa, TraceLevel_Info);

	for (int i = 0; i < Table; i++)
	{
		aa.Format(TEXT("首次分配第%d桌随机分配%d个机器人.之后每%d秒重新更新每桌机器人 该信息将不再打印"), (i + 1), DistributingRobot[i], TIME_INTERLUDE_ANDROID);
		CTraceService::TraceString(aa, TraceLevel_Info);
	}
////////////////////////////////////////////////////////////////////////////
	//设置时间
	...
	m_pITimerEngine->SetTimer(IDI_INTERLUDE_ANDROID, TIME_INTERLUDE_ANDROID * 1000L, TIMES_INFINITY, NULL);
#endif

	//m_pITimerEngine->SetTimer(IDI_LOAD_ANDROID_USER,TIME_LOAD_ANDROID_USER*260L, TIMES_INFINITY,NULL);
	//m_pITimerEngine->SetTimer(IDI_DISTRIBUTE_ANDROID,TIME_DISTRIBUTE_ANDROID*600L,TIMES_INFINITY,NULL);
	...
	int nAndroidMessageCount=10000; 
	nAndroidMessageCount=IniData.ReadInt(TEXT("Time"),TEXT("Elapse"),10000);

	//设置时间
	m_pITimerEngine->SetTimer(IDI_GAME_SERVICE_PULSE,1000L,TIMES_INFINITY,NULL);

	m_pITimerEngine->SetTimer(IDI_ON_ANDROID_SEND_TRUMPT,nAndroidMessageCount*1000,TIMES_INFINITY,NULL);
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	if ((dwTimerID>=IDI_MAIN_MODULE_START)&&(dwTimerID<=IDI_MAIN_MODULE_FINISH))
	{
		...
		case IDI_INTERLUDE_ANDROID://每60秒每桌的机器人将再次分配
			{
				//CString aa;
				if ((Chair < MAX_CHAIR) && (StartMode == START_MODE_TIME_CONTROL))//如果是时间控制模式
				{
					//aa.Format(TEXT("启动随机分配"));
					//CTraceService::TraceString(aa, TraceLevel_Info);
					for (int i = 0; i < Table; i++){
					Sleep(1);
					if (Chair == 1){
		...
		case IDI_DISTRIBUTE_ANDROID:	//分配机器
			{
				...
				{
					//时间处理
					m_AndroidUserManager.OnEventTimerPulse(IDI_REBOT_MODULE_START,2);
					...
					//模拟处理(不是防作弊模式、配置允许机器人占位) 被动陪打
					if (bAllowAndroidSimulate==true && bAllowAvertCheatMode==false)
					{
						//机器状态
						tagAndroidUserInfo AndroidSimulate;  //相互模拟
						...
						if (AndroidSimulate.wFreeUserCount>0)
						{
							for (WORD i=0;i<8;i++)
							   ...
							   //随机桌子
								...
								//桌子已经坐满不考虑
								//if ((wUserSitCount>=pTableFrame->GetChairCount())&&(m_pGameServiceAttrib->wChairCount<MAX_CHAIR)) continue;
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								//分配判断
								if (TableUserInfo.wTableUserCount>0) continue;
								if (StartMode == START_MODE_TIME_CONTROL || StartMode == START_MODE_ALL_READY)
								{
									if ((m_pGameServiceAttrib->wChairCount<MAX_CHAIR) && (wUserSitCount >= DistributingRobot[i]))continue;//过滤
								}
								else
								{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								//预留3-8个人不陪打
								int tempFreeUserCount=AndroidSimulate.wFreeUserCount;
								if(tempFreeUserCount<(m_AndroidUserManager.GetAndroidCount()/5))
								{
									tempFreeUserCount-=(rand()%5+3);
								}
								//人数用完直接退出
								if(tempFreeUserCount<=0) break;
								//一次陪坐当前桌子
								for(WORD wAndroidUser=0; wAndroidUser<tempFreeUserCount; ++wAndroidUser)
								{
									IAndroidUserItem * pIAndroidUserItem=AndroidSimulate.pIAndroidUserFree[wAndroidUser];
									...
									//相差分数不陪玩
									//bool bPlay=true;
									//SCORE lMinTakeScore=pIAndroidUserItem->GetAndroidParameter()->lMinTakeScore;						
									//SCORE lMaxTakeScore=pIAndroidUserItem->GetAndroidParameter()->lMaxTakeScore;
									//IServerUserItem *pIOtherServerUserItem = NULL;
									//for (WORD wOtherChairID=0;wOtherChairID<pTableFrame->GetChairCount();++wOtherChairID)
									//{
									//	pIOtherServerUserItem=pTableFrame->GetTableUserItem(wOtherChairID);
									//	if (pIOtherServerUserItem == NULL) continue;
									//	if (pIOtherServerUserItem->GetUserScore()<lMinTakeScore||pIOtherServerUserItem->GetUserScore()>lMaxTakeScore)
									//	{
									//		bPlay=false;
									//		break;
									//	}
					...
					{
						//被动状态
						tagAndroidUserInfo AndroidPassivity; //被动陪打
							...
							if (AndroidPassivity.wFreeUserCount>0)
							{
								...
								else
								{
									...
									for (WORD wTableID=0;wTableID<m_pGameServiceOption->wTableCount;wTableID++)
									{
										...
										//if (TableUserInfo.wTableUserCount>0) continue;
										//桌子已经坐满不考虑
										if ((wUserSitCount>=pTableFrame->GetChairCount())&&(m_pGameServiceAttrib->wChairCount<MAX_CHAIR))
										{
											continue;
										}
										...
										//预留3-8个人不陪打
										int tempFreeUserCount=AndroidPassivity.wFreeUserCount;
										//if(tempFreeUserCount<(m_AndroidUserManager.GetAndroidCount()/5))
										//{
										//	tempFreeUserCount-=(rand()%5+3);
										//}
										//人数用完直接退出
										/*if(tempFreeUserCount<=0)
											{
											break;
										}*/
										//一次陪坐当前桌子
										for(WORD wAndroidUser=0; wAndroidUser<tempFreeUserCount; ++wAndroidUser)
												{
											IAndroidUserItem * pIAndroidUserItem=AndroidPassivity.pIAndroidUserFree[wAndroidUser];
									...
									if (bPassivitySuss==true)return true;
								}
							}
						}
					}
					...
					{
						//主动状态
						tagAndroidUserInfo AndroidInitiative; //主动坐下
						...
						else
						{
							...
							{
							//	WORD wTempTableCount=m_pGameServiceOption->wTableCount>50?m_pGameServiceOption->wTableCount*2/3:m_pGameServiceOption->wTableCount;
							//	WORD wIndex=rand()%wTempTableCount;
								//for (INT_PTR i=wIndex;i<wTempTableCount+wIndex;i++)
								for (INT_PTR i = 0; i<m_pGameServiceOption->wTableCount; i++)
								{
									INT_PTR ai = rand() % m_pGameServiceOption->wTableCount;//智能随机坐下
									//获取桌子
									CTableFrame * pTableFrame = m_TableFrameArray[ai];
									if ((pTableFrame->IsGameStarted()==true)&&(bAllowDynamicJoin==false)) continue;
									...
									if (StartMode == START_MODE_TIME_CONTROL || StartMode == START_MODE_ALL_READY)
									{
										if ((m_pGameServiceAttrib->wChairCount < MAX_CHAIR) && (wUserSitCount >= DistributingRobot[ai]))continue;//过滤
									}
									else{
					...
					//百人游戏
					if (m_pGameServiceAttrib->wChairCount >= 50)
					{
						return true;
					}
					//起立处理
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						int as = 0;
						//处理有真实玩家的桌子
						for (WORD k = 0; k<pTableFrame->GetChairCount(); k++)
						{
							//获取用户
							IServerUserItem * pIServerUserIt = pTableFrame->GetTableUserItem(k);
							if (pIServerUserIt == NULL) continue;
							//存在真实玩家机器人不能说走就走
							if ((pIServerUserIt->IsAndroidUser() == false))
							{
								as = rand() % 1000;   /*求出0到999之间的随机数*/ //获得起立几率
								//CString aa;
								//aa.Format(TEXT("A第%d桌有真实玩家 获得起立种子为%d"), i + 1, as);
								//CTraceService::TraceString(aa, TraceLevel_Info);
							}
						}
						if ((m_pGameServiceAttrib->wChairCount < MAX_CHAIR) && (StartMode == START_MODE_TIME_CONTROL ) && (wUserSitCount >= DistributingRobot[i]))//如果是时间控制的游戏不是百人游戏
						{
							//CString aa;
							//aa.Format(TEXT("当前人数%d,分配机器数%d"), wUserSitCount, DistributingRobot[i]);
							//CTraceService::TraceString(aa, TraceLevel_Info);
							if (as>111)continue;//留下几率
							if (wUserSitCount == 0)continue;
							//这种情况要下掉机器人
						}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						...
						{
							WORD ai = rand() % pTableFrame->GetChairCount();//智能起立
							//获取用户
							IServerUserItem * pIServerUserItem=pTableFrame->GetTableUserItem(ai);
							if (pIServerUserItem==NULL) continue;
					...
					for (INT_PTR i=nIndex;i<m_pGameServiceOption->wTableCount+nIndex;++i)
					{
						...
						//桌子状况
						tagTableUserInfo TableUserInfo;
						WORD wUserSitCount=pTableFrame->GetTableUserInfo(TableUserInfo);//当前人数
///////////////////////////////////////////////////////////////////////////////////////////////////////
						int as = 0;
						//处理有真实玩家的桌子
						for (WORD k = 0; k<pTableFrame->GetChairCount(); k++)
						{
							//获取用户
							IServerUserItem * pIServerUserIte = pTableFrame->GetTableUserItem(k);
							if (pIServerUserIte == NULL) continue;

							//存在真实玩家机器人不能说走就走
							if ((pIServerUserIte->IsAndroidUser() == false))
							{
								as = rand() % 1000;   /*求出0到999之间的随机数*/ //获得起立几率
								//CString aa;
								//if (as<111)
								//	aa.Format(TEXT("第%d桌有真实玩家 获得留下种子为%d"), i + 1, as);
								//CTraceService::TraceString(aa, TraceLevel_Info);
							}
						}
						...
						if ((m_pGameServiceAttrib->wChairCount < MAX_CHAIR) && (StartMode == START_MODE_TIME_CONTROL) && (wUserSitCount >= DistributingRobot[i]))//如果是时间控制的游戏不是百人游戏
						{
							if (as>111)continue;//这种情况留下的几率会很高
							if (wUserSitCount == 0)continue;
							//这种情况要下掉机器人
						}
						else
						{
							if (pTableFrame->IsGameStarted() == true) continue;//如果是游戏状态留下
						}
///////////////////////////////////////////////////////////////////////////////////////////////////////
						//用户过虑
						bool bRand = ((rand()%100)>50);
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GR_BANK_DETAIL_RESULT:
		{
			return OnDBPCBankDetailResult(dwContextID, pData, wDataSize);
		}
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	UserScore.UserScore.lModifyScore=pUserInfo->lModifyScore;

	pUserInfo->lModifyScore=0;
...
//明细结果
bool CAttemperEngineSink::OnDBPCBankDetailResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize%sizeof(DBO_GR_BankDetailResult)==0);
	if (wDataSize%sizeof(DBO_GR_BankDetailResult)!=0) return false;
	...
	//变量定义
	WORD wItemCount=wDataSize/sizeof(DBO_GR_BankDetailResult);
	DBO_GR_BankDetailResult * pBankDetailResult=(DBO_GR_BankDetailResult *)pData;
	...
	for (WORD i=0;i<wItemCount;i++)
	{
		...
		//发送数据
		if ((wSendSize+sizeof(DBO_GR_BankDetailResult))>sizeof(cbDataBuffer))
		{
			SendData(pIServerUserItem,MDM_GR_INSURE,SUB_GR_BANK_DETAIL_RESULT,cbDataBuffer,wSendSize);
			wSendSize=0;
		}

		//拷贝数据
		CMD_GR_BankDetailResult* pDetailResult=(CMD_GR_BankDetailResult *)(cbDataBuffer+wSendSize);
		pDetailResult->cbState = (pBankDetailResult+i)->cbState;
		lstrcpyn(pDetailResult->szNickName, (pBankDetailResult+i)->szNickName, CountArray(pDetailResult->szNickName));
		pDetailResult->dwGameID = (pBankDetailResult+i)->dwGameID;
		pDetailResult->lSwapScore = (pBankDetailResult+i)->lSwapScore;
		lstrcpyn(pDetailResult->szDateTime, (pBankDetailResult+i)->szDateTime, CountArray(pDetailResult->szDateTime));
		wSendSize+=sizeof(CMD_GR_BankDetailResult);
	}

	//发送剩余
	if (wSendSize>0) SendData(pIServerUserItem,MDM_GR_INSURE,SUB_GR_BANK_DETAIL_RESULT,cbDataBuffer,wSendSize);
...
bool CAttemperEngineSink::OnTCPNetworkMainInsure(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GR_QUERY_BANK_DETAIL:		//查询记录
		{
			return OnTCPNetworkSubQueryInsureRecord(pData,wDataSize,dwSocketID);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonUserID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//密码判断
	if(pBindParameter->dwClientAddr!=0L && lstrlen(pLogonUserID->szPassword)==0)
	{
		//发送失败
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonMobile(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//密码判断
	if(pBindParameter->dwClientAddr!=0L && lstrlen(pLogonMobile->szPassword)==0)
	{
		//发送失败
		SendLogonFailure(TEXT("很抱歉，您的登录密码错误，不允许继续进入！"),LOGON_FAIL_SERVER_INVALIDATION,dwSocketID);
		return true;
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//密码判断
	if(pBindParameter->dwClientAddr!=0L && lstrlen(pLogonAccounts->szPassword)==0)
	{
		//发送失败
		SendLogonFailure(TEXT("很抱歉，您的登录密码错误，不允许继续进入！"),LOGON_FAIL_SERVER_INVALIDATION,dwSocketID);
		return true;
...
int CAttemperEngineSink::OnPropertyBuy(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (pPropertyBuy->cbConsumeScore==TRUE)
	{
		...
		{
			SendPropertyFailure(pIServerUserItem,TEXT("您的欢乐豆余额不足，道具购买失败！"),0L,pPropertyBuy->cbRequestArea);
			return RESULT_FAIL;
		}
	}
	else
...
bool CAttemperEngineSink::OnTCPAndroidSendTrumpet()
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	////顺序发的
// 	static int current = 1;//消息编号从1开始
// 	CString entryStr;
// 	entryStr.Format(_T("Msg%d"),current);
// 	IniData.ReadString(TEXT("Message"),entryStr,NULL,szMessage,CountArray(szMessage));
// 
// 	entryStr.Format(_T("Name%d"),current);
// 	IniData.ReadString(TEXT("NickName"),entryStr,NULL,szName,CountArray(szName));
// 
// 	current++;
// 	int maxCount=IniData.ReadInt(TEXT("MessageCount"),TEXT("Count"),1);
// 	if(current>maxCount)
// 		current=1;
	////顺序发结束
	//////////////////////////////////////////////////////////////////////////////////////////////////
...
bool CAttemperEngineSink::OnTCPNetworkSubSaveScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (pSaveScoreRequest->lSaveScore>lConsumeQuota)
	{
		if (lConsumeQuota<lUserWholeScore)
		{
			//构造提示
			TCHAR szDescribe[128]=TEXT("");
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("由于您正在游戏中，欢乐豆可存入额度为 %I64d，存入操作失败！"),lConsumeQuota);
		...
		{
			//发送数据
			SendInsureFailure(pIServerUserItem,TEXT("您的欢乐豆余额不足，存入操作失败！"),0L,pSaveScoreRequest->cbActivityGame);
		}
...
//查询记录
bool CAttemperEngineSink::OnTCPNetworkSubQueryInsureRecord(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
	ASSERT(wDataSize==sizeof(CMD_GR_QueryBankDetail));
	if (wDataSize!=sizeof(CMD_GR_QueryBankDetail)) return false;

	//处理消息
	CMD_GR_QueryBankDetail * pQueryBankDetail=(CMD_GR_QueryBankDetail *)pData;

	//变量定义
	DBR_GR_QueryBankDetail QueryIndividual;
	ZeroMemory(&QueryIndividual,sizeof(QueryIndividual));

	//构造数据
	QueryIndividual.dwUserID=pQueryBankDetail->dwUserID;
	QueryIndividual.cbTransferIn=pQueryBankDetail->cbTransferIn;

	//投递请求
	m_pIDBCorrespondManager->PostDataBaseRequest(QueryIndividual.dwUserID,DBR_GR_QUERY_BANK_DETAIL,dwSocketID,&QueryIndividual,sizeof(QueryIndividual));
...
bool CAttemperEngineSink::OnTCPNetworkSubOptionServer(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (bModifyServer==true)
	{
		...
		ServerModify.lMinEnterScore=m_pGameServiceOption->lMinEnterScore;
