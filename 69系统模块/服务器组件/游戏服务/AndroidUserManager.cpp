...
#define TIME_LOAD_ANDROID_INFO		120L								//加载用户
...
CAndroidUserManager::CAndroidUserManager()
{
	...
	m_wDBStockCount=0;
	ZeroMemory(&m_DBAndroidParameter,sizeof(m_DBAndroidParameter));
	//设置索引
...
bool CAndroidUserManager::SetAndroidStock(tagAndroidParameter AndroidParameter[], WORD wStockCount)
{
	...
	for (WORD i=0;i<wStockCount;i++)
	{
		...
		m_DBAndroidParameter[m_wDBStockCount++]=AndroidParameter[i];
...
IAndroidUserItem * CAndroidUserManager::CreateAndroidUserItem(tagAndroidParameter & AndroidParameter)
{
	...
	LogonUserID.dwProcessVersion=m_pGameServiceAttrib->dwClientVersion;
		...
		LogonUserID.dwProcessVersion=m_pGameParameter->dwClientVersion;
...
// 					if (cbEnlistStatus!=MS_NULL) continue;
					...
					//离开判断
					bool bLeaveServer=true;
					//如果在数据库机器人库存表里不存在，则退出
					for(WORD wIndex=0;wIndex<m_wDBStockCount;wIndex++)
					{
						//判断是否存在
						if(pIServerUserItem->GetUserID()==m_DBAndroidParameter[wIndex].dwUserID) 
						{
							//存在，不退出，结束
							bLeaveServer=false;
							break;
					...
					//小于最小积分范围，退出
					if(pIServerUserItem->GetUserScore()<pAndroidParameter->lMinTakeScore) bLeaveServer=true;
					//大于最大积分范围，退出
				//	if(pIServerUserItem->GetUserScore()>pAndroidParameter->lMaxTakeScore) bLeaveServer=true;
