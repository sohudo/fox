...
bool CGameMatchSink::OnEventGameStart(ITableFrame *pITableFrame, WORD wChairCount)
{
	if(m_pGameEventSink!=NULL)
		return m_pGameEventSink->OnEventGameStart(pITableFrame, wChairCount);
	return false;
}
...
{
	if(m_pGameEventSink!=NULL)
		return m_pGameEventSink->OnEventGameEnd(pITableFrame,wChairID,pIServerUserItem,cbReason);
	return false;
}
