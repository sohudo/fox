...
#define UPDATE_PROCESS_NAME			TEXT("AutoUpdate")
...
typedef struct tagDownloadTask {
	...
	DWORD   dwFileSize;
	BOOL	bFinishd;
	TCHAR	szRemoteFile[128];
	TCHAR	szDirectory[128];
	TCHAR	szLocalFile[128];
} DOWNLOADTASK;
...
protected:
	...
	//字符串替换
	const char *ChangeChar(char * pDest,size_t size,char *pStr,char cSrc,char cDest);
};
