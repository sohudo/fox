...
INT CUserListControl::SortItemData(LPARAM lParam1, LPARAM lParam2, WORD wColumnIndex, bool bAscendSort)
{
	...
	{
    case UD_TABLE_GRADE:	//当桌成绩
		{
			//获取变量
			SCORE lBanker1=pIClientUserItem1->GetTableGrade();
			SCORE lBanker2=pIClientUserItem2->GetTableGrade();
...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
	...
	switch (m_cbDataDescribe[wColumnIndex])
	{
     case UD_TABLE_GRADE:	//用户成绩
		{
			_sntprintf(pszString,wMaxCount,TEXT("%I64d"),pIClientUserItem->GetTableGrade());
			break;
		}
...
VOID CUserListControl::GetItemColor(LPDRAWITEMSTRUCT lpDrawItemStruct, COLORREF&crColorText, COLORREF&crColorBack)
{
	...
	else if (pIClientUserItem->GetMemberOrder()!=0)
	{
		...
		switch(pIClientUserItem->GetMemberOrder())
		{
		case 1:
			{
				crColorText=RGB(0,0,255);	//lan
				break;
			}
		case 2:
			{
				crColorText=RGB(255,255,0);	//huang
				break;
			}
		case 3:
			{
				crColorText=RGB(255,255,255);//bai
				break;
			}
		case 4:
			{
				crColorText=RGB(255,0,0);	//hong
				break;
			}
		}
	}
...
//设置列表
bool CUserListControl::SetColumnDescribe(tagColumnItem ColumnItem[], WORD wColumnCount,int iFilterIndex)
{
	//构造判断
