...
interface IInsureCounterAction
{
	...
	//查询记录
	virtual VOID PerformQueryRecord(BYTE cbTransferIn)=NULL;
	//修改密码
...
public:
	...
	CSkinHyperLink                  m_btDetail;

protected:
	CListCtrl m_ListDetail;
	CSkinHeaderCtrl m_HeadCtrl;
	CSkinButton m_btnQueryOut; // 查询转出
	CSkinButton m_btnQueryIn; //转入
...
public:
	//插入记录
	void InsertBankDetail(LPCTSTR lpszNickName, DWORD dwGameID, SCORE lSwapScore, LPCTSTR lpszTime, BYTE cbTransferIn);

	void OnBnClickedQueryOut();
	void OnBnClickedQueryIn();
...
protected:
	...
	bool SaveToCliboard(HBITMAP hBitmap);
	//绘画消息
...
protected:
	...
	//查询记录
	virtual VOID PerformQueryRecord(BYTE cbTransferIn);
	//修改密码
