...
bool CMissionLogon::OnEventMissionLink(INT nErrorCode)
{
	...
	{
		if(m_bRegisterCheck!=CHECK_NULL)
		{
			SendCheckPacket(); 
			return true;
		}
...
bool CMissionLogon::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
	if (Command.wMainCmdID==MDM_GP_LOGON)
	{
		...
		{
		case SUB_GP_NEED_EXTERN:	//额外信息
			{
				return OnSocketSubLogonNeedExtern(pData,wDataSize);
			}
		...
		case SUB_GP_REGISTER_CHECKRESULT: //检查结果
			{
				return OnSocketRegisterResult(pData,wDataSize);
			}
		}
	}
...
//执行登录
bool CMissionLogon::PerformLogonMission(bool bRemPassword,BYTE cbRegisterCheck)
{
	//效验状态
	ASSERT(GetActiveStatus()==false);
	if (GetActiveStatus()==true) return false;

	m_bRegisterCheck = cbRegisterCheck;
...
//
bool CMissionLogon::OnSocketSubLogonNeedExtern(VOID * pData, WORD wDataSize)
{
...
bool CMissionLogon::OnSocketSubLogonFinish(VOID * pData, WORD wDataSize)
{
	...
	CPlatformFrame::GetInstance()->WebBrowse(TEXT("http://www.85jn.com/service.html"),true);
...
//注册检测结果
bool CMissionLogon::OnSocketRegisterResult(VOID * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize==sizeof(CMD_GP_CheckResult));
	if (wDataSize!=sizeof(CMD_GP_CheckResult)) return false;

	//转换数据
	CMD_GP_CheckResult* pCheckResult = (CMD_GP_CheckResult*)pData;

	//更新注册显示
	if(pCheckResult->cbCheckType==CHECK_ACCOUNT)
	{
		if(pCheckResult->cbResult==0)
		{
			m_pDlgRegister->m_strTipString[0]=TEXT("恭喜，账号可用！");
			m_pDlgRegister->m_cbFlag[0]=2;
		}
		else
		{
			m_pDlgRegister->m_strTipString[0]=TEXT("账号已存在");
			m_pDlgRegister->m_cbFlag[0]=0;
		}

		m_pDlgRegister->InvalidateRect(NULL,TRUE);
	}
	else if(pCheckResult->cbCheckType==CHECK_NICK)
	{
		if(pCheckResult->cbResult==0)
		{
			m_pDlgRegister->m_strTipString[1]=TEXT("恭喜，昵称可用！");
			m_pDlgRegister->m_cbFlag[1]=2;
		}
		else
		{
			m_pDlgRegister->m_strTipString[1]=TEXT("昵称已存在");
			m_pDlgRegister->m_cbFlag[1]=0;
		}
...
//发送检测包
bool CMissionLogon::SendCheckPacket()
{
	//变量定义
	WORD wPacketSize,wSubCmdID;
	BYTE cbBuffer[SOCKET_TCP_PACKET];
	...
	//构建数据
	wSubCmdID = SUB_GP_REGISTER_CHECK;
	wPacketSize = m_pDlgRegister->ConstructCheckPacket(cbBuffer,sizeof(cbBuffer),m_bRegisterCheck);
