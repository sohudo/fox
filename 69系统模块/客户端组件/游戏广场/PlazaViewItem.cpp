...
BOOL CPlazaViewItem::OnInitDialog()
{
	...
	m_btWebLast.ShowWindow(SW_HIDE);
	m_btWebNext.ShowWindow(SW_HIDE);
	m_btWebStop.ShowWindow(SW_HIDE);
	m_btWebHome.ShowWindow(SW_HIDE);
	m_btWebReload.ShowWindow(SW_HIDE);
...
BOOL CPlazaViewItem::OnEraseBkgnd(CDC * pDC)
{
	...
	ImageTitleL.LoadFromResource(hInstance,IDB_BROWSER_L);
	ImageTitleM.LoadFromResource(hInstance,IDB_BROWSER_M);
	ImageTitleR.LoadFromResource(hInstance,IDB_BROWSER_R);

	pDC->FillSolidRect(0,0,rcClient.Width(),ImageTitleM.GetHeight(),RGB(199,220,231));

	//绘画背景
	ImageTitleL.BitBlt(pDC->GetSafeHdc(),0,2);

	//填充中间
	for (INT nXPos=ImageTitleL.GetWidth();nXPos<rcClient.Width();nXPos+=ImageTitleM.GetWidth())
	{
		ImageTitleM.BitBlt(pDC->GetSafeHdc(),nXPos,2);
	}

	//绘画背景
	ImageTitleR.BitBlt(pDC->GetSafeHdc(),rcClient.Width()-ImageTitleR.GetWidth(),2);

	pDC->SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());
	pDC->SetTextColor(RGB(55,93,110));
	pDC->SetBkMode(1);
	pDC->TextOut(ImageTitleL.GetWidth()+6,8,TEXT("构筑和谐社会 打造绿色网络 禁止赌博色情 文明健康游戏 祝您在游戏中开心每一天!"));
...
VOID CPlazaViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//浏览控件
	//CRect rcWebBrowse(-2,2,nWidth+1,nHeight+5);
	CRect rcWebBrowse(-2,2,nWidth+5,nHeight+5);
