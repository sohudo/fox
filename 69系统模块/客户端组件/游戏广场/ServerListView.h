...
typedef vector<tagKindState>		CKindStateArray;
...
protected:
	...
	CFont							m_FontUnderLine;					//下划线字
	CPngImage						m_ImageArrow;						//箭头图标
	CBitImage						m_ImageType;						//类型图片
	CPngImage						m_ImageTypeArrow;					//类型箭头
	CPngImage						m_ImageKindLine;
	CPngImage						m_ImageKindStar;					//星星
	CPngImage						m_ImageKindState;					//状态
	CPngImage						m_ImageServerState;	
...
protected:
	...
	CKindStateArray					m_KindStateArray;				//
...
private:
	...
	//绘制类型
	VOID DrawTreeType(CDC * pDC, CRect & rcClient, bool bDrawSelected, bool bDrawChildren, bool bHover,CGameListItem* pListItem,HTREEITEM hItemCurrent);
	//绘制游戏
	VOID DrawTreeKind(CDC * pDC, CRect & rcClient, bool bDrawSelected, bool bDrawChildren, bool bHover,CGameListItem* pListItem,HTREEITEM hItemCurrent);
	//绘制房间
	VOID DrawTreeServer(CDC * pDC, CRect & rcClient, bool bDrawSelected, bool bDrawChildren, bool bHover,CGameListItem* pListItem,HTREEITEM hItemCurrent);
...
protected:
	...
	//获取房间负载信息
	WORD GetLoadInfoOfServer(tagGameServer * pGameServer);
	WORD GetLoadInfoOfServer(DWORD dwOnLineCount, DWORD dwMaxCount);
	//加载记录
