...
BOOL CDlgStatus::OnInitDialog()
{
	...
	//设置按钮
	m_btGiveUp.SetWindowPos(NULL,100,50,0,0,SWP_NOSIZE|SWP_NOZORDER);
...
BOOL CDlgStatus::OnEraseBkgnd(CDC * pDC)
{
	...
	SizeStatusLine.SetSize(ImageStatusLine.GetWidth(),ImageStatusLine.GetHeight()/2-1);
	SizeStatusPoint.SetSize(ImageStatusPoint.GetWidth(),ImageStatusPoint.GetHeight());

	//绘画进度
	ImageStatusLine.DrawImage(pBufferDC,80,45,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawImage(pBufferDC,80,45,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	ImageStatusPoint.DrawImage(pBufferDC,80+nXPoint-SizeStatusPoint.cx/2,49+(SizeStatusLine.cy-SizeStatusPoint.cy)/2);
