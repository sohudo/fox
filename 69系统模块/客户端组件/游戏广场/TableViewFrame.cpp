...
VOID CTableView::DrawTableView(CDC * pDC, DWORD dwServerRule, CTableResource * pTableResource)
{
		...
		for (WORD i=0;i<m_TableAttribute.wChairCount;i++)
		{
			...
			if (pTableResource->m_bShowUser==true)
			{
				...
				if ((bAvertCheatMode==false)&&(m_TableAttribute.pIClientUserItem[i]!=NULL))
				{
					...
					{
						//计算位置
						INT nXExcursion=(rcChair.Width()-32)/2;
						INT nYExcursion=(rcChair.Height()-32)/2;
						...
						pIFaceItemControl->DrawFaceTableNormal(pDC,rcChair.left+nXExcursion,rcChair.top+nYExcursion,pIClientUserItem->GetGender(),m_pITableViewFrame->GetUserLevel(pIClientUserItem));
					}
					else
					{
						//自定头像
	...
	if (pTableResource->m_bShowTableID==true)
	{
		...
		//};
		//构造路径
		TCHAR szTableNumber[10]=TEXT("");
		_sntprintf(szTableNumber,CountArray(szTableNumber),TEXT("- %d -"),m_TableAttribute.wTableID+1);
		pDC->SetTextColor(RGB(255,255,255));
		pDC->TextOut(pTableResource->m_ptTableID.x,pTableResource->m_ptTableID.y,szTableNumber);
		//CRect rcNumber(0,0,);
		////构造位置
		//rcNumber.left=m_ptTableID;
		//rcNumber.right=rcName.right;
...
VOID CTableView::DrawUserAvatar(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem, CTableResource * pTableResource)
{
	...
	IFaceItemControl * pIFaceItemControl=CFaceItemControl::GetInstance();
	pIFaceItemControl->DrawFaceTableNormal(pDC,nXPos,nYPos,pIClientUserItem->GetGender(),m_pITableViewFrame->GetUserLevel(pIClientUserItem));
	...
	//{
		...
		//		pTableResource->m_ImageUserFace.BitBlt(pDC->m_hDC,nXPos,nYPos,nWidth,nHeight,(pIClientUserItem->GetGender()==GENDER_FEMALE?0:1)*FACE_CX,0);
		//	}
		//	else
		//	{
		//		////调整参数
		...
		//		//INT nXImagePos=wFaceID%wLineCount;
		//		//INT nYImagePos=(wFaceID/wLineCount)*2L;
		//		pTableResource->m_ImageUserFace.BitBlt(pDC->m_hDC,nXPos,nYPos,nWidth,nHeight,(pIClientUserItem->GetGender()==GENDER_FEMALE?0:1)*FACE_CX,FACE_CY);
		//	}
		//}
		//else
		//{
		//	IFaceItemControl * pIFaceItemControl=CFaceItemControl::GetInstance();
		//	pIFaceItemControl->DrawFaceNormal(pDC,nXPos,nYPos,pIClientUserItem->GetGender());
		//}
	//}
...
CTableViewFrame::~CTableViewFrame()
{
	...
	if(m_pDlgUserInfo!=NULL)delete m_pDlgUserInfo;
	m_pDlgUserInfo=NULL;
...
//获取等级
WORD CTableViewFrame::GetUserLevel(IClientUserItem * pIClientUserItem)
{
	return m_pIGameLevelParser->GetLevelNum(pIClientUserItem);
}
//游戏标志
	...
	if (m_wDownTableID!=INVALID_TABLE) 
	{
		...
		if (pITableView!=NULL)
		{
			...
			if(m_pDlgUserInfo!=NULL)
		        m_pDlgUserInfo->ShowUserInfo(NULL);
		}
	}
	...
	if ((m_bRMouseDown==false)&&(m_bLMouseDown==false))
	{
		...
		if (((wNewTableID!=m_wDownTableID)||(wNewChairID!=m_wDownChairID))&&((wNewTableID!=INVALID_CHAIR)||(m_wDownChairID!=INVALID_CHAIR)))
		{
			...
			if ((wNewTableID!=INVALID_TABLE)&&(wNewChairID!=INVALID_CHAIR))
			{
				...
				if(GetClientUserItem(wNewTableID,wNewChairID)!=NULL)
				{
					if(m_pDlgUserInfo==NULL)
					{
						m_pDlgUserInfo=new CDlgUserInfo;
						m_pDlgUserInfo->Create(IDD_DLG_USER_INFO,this);
						m_pDlgUserInfo->SetGameLevelParser(m_pIGameLevelParser);
					}
                    CPoint MousePoint1;
		            GetCursorPos(&MousePoint1);
					m_pDlgUserInfo->SetWindowPos(NULL,MousePoint1.x,MousePoint1.y,0,0,SWP_NOSIZE);
					m_pDlgUserInfo->ShowUserInfo(GetClientUserItem(wNewTableID,wNewChairID),m_szServerName);
				}
			}
		}
