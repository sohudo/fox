...
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszClientName, WORD wKindID, WORD wServerID)
{
	...
	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad.aspx?KindID=%ld"),szDownloadLink,wKindID);
...
bool CDownLoadSink::PerformInstall()
{
	...
	//	_sntprintf(szCommandLine,CountArray(szCommandLine),TEXT("%s /VERYSILENT"),szTargetName);
	    _sntprintf(szCommandLine,CountArray(szCommandLine),TEXT("%s /DownLoad/"),szTargetName);
	else
		_sntprintf(szCommandLine,CountArray(szCommandLine),TEXT("%s"),szTargetName);
	//解压文件
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	TCHAR szWorkDirectory[MAX_PATH] = TEXT("");
	//	CWHService::GetWorkDirectory(szWorkDirectory, CountArray(szWorkDirectory));//获得程序当前目录
	HZIP hz = OpenZip(szTargetName, 0, ZIP_FILENAME);
	ZIPENTRYW ze;
	GetZipItem(hz, -1, &ze);
	...
	for (int i = 0; i < numitems; i++)
	{
		...
		int ret = UnzipItem(hz, i, ze.name, 0, ZIP_FILENAME);
		if (ret != 0)
		{
			_tprintf(TEXT("ret:%d file:%s %x\r\n"), ret, ze.name, ze.attr);
		}
		else
		{
			_tprintf(TEXT("ret:%d file:%s %x\r\n"), ret, ze.name, ze.attr);
		...
		{
			_tprintf(TEXT("set file chmod error!\r\n"));
			//break;
		}
	}
	CloseZip(hz);
	if (_waccess(szTargetName, 0) == -1)
	{
		ASSERT(FALSE);
		return false;
	}
...
VOID CDlgDownLoad::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	for (INT i=0;i<m_wViewLineCount;i++)
	{
		...
		if (wDrawIndex<m_DownLoadSinkArray.GetCount())
		{
          ...
          {  
					dwDownScale=pDownLoadSink->m_dwDownLoadSize*10L/(pDownLoadSink->m_dwTotalFileSize/10); 
          }  
