...
//常量定义
#define PROPERTY_CY					89//89									//游戏道具
#define PUBLICIZE_CY				0//80									//游戏广告
...
BOOL CServerViewItem::OnInitDialog()
{
	...
	m_HorSplitter.SetSplitterColor(RGB(193,215,238));
	...
	m_TableViewFrame.SetGameLevelParser(m_GameLevelParserModule.GetInterface());
	...
	case IDC_BT_TABLE_BUTTON_5:		//更多按钮
		{
			...
			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/GameRules/%ld.html"),szPlatformLink,m_GameKind.wGameID);
	...
	case IDM_COPY_USER_NICKNAME:	//拷贝昵称
		{
			...
			case IDM_COPY_USER_NICKNAMEID:	//拷贝ID
		{
		
	     ASSERT(m_MenuUserItemArray.GetCount()>0);
        if (m_MenuUserItemArray.GetCount()==0) return TRUE;
          IClientUserItem * pIClientUserItem=m_MenuUserItemArray[0];
          TCHAR szGameID[64] = _T("");
         _sntprintf(szGameID, CountArray(szGameID), _T("%u"), pIClientUserItem->GetGameID());
         CWHService::SetClipboardString(szGameID);
          return TRUE;
...
VOID CServerViewItem::TrackUserItemMenu(IClientUserItem * pIClientUserItem)
{
    ...
    UserInfoMenu.AppendMenu(IDM_COPY_USER_NICKNAMEID,TEXT("复制用户ID"));
	//设置交谈
...
VOID CServerViewItem::ShowInsureView()
{
	...
	DlgInsureServer.SetInsurePassword(CGlobalUnits::GetInstance()->m_szInsurePassword);
	...
	case SUB_GR_CONFIG_SERVER:		//房间配置
		{
			...
			TCHAR szServerName[100]=TEXT("");
			_sntprintf(szServerName,CountArray(szServerName),TEXT("%s-%s"),m_GameKind.szKindName,m_GameServer.szServerName);
			m_TableViewFrame.SetServerName(szServerName);
...
bool CServerViewItem::OnSocketMainInsure(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	if(wSubCmdID!=SUB_GR_BANK_DETAIL_RESULT && *pActivityGame==TRUE)
	{
...
bool CServerViewItem::OnSocketSubLogonSuccess(VOID * pData, WORD wDataSize)
{
	...
	m_DlgStatus.ShowStatusWindow(TEXT(""));
	////构造地址
...
bool CServerViewItem::OnSocketSubMatchFee(VOID * pData, WORD wDataSize)
{
	...
	if(dwMatchFee>0)
	{
		...
		_sntprintf(szDescribe,CountArray(szDescribe),TEXT("参赛将扣除报名费 %ld 欢乐豆，确认要参赛吗？"),dwMatchFee);
...
VOID CServerViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	rcVorSplitter.left=rcVorSplitter.right;
	...
	DeferWindowPos(hDwp,m_HorSplitter,NULL,rcVorSplitter.right+1,rcHorSplitter.top,nWidth-rcVorSplitter.right,rcHorSplitter.Height(),uFlags);
	...
	DeferWindowPos(hDwp,m_btTableButton3,NULL,rcTableArea.right-142,3,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btTableButton5,NULL,rcTableArea.right-206,3,0,0,uFlags|SWP_NOSIZE);
...
BOOL CServerViewItem::OnEraseBkgnd(CDC * pDC)
{
	...
	pDC->TextOut(24,7,TEXT("构筑和谐社会 打造绿色网络 禁止赌博色情 文明健康游戏 祝您在游戏中开心每一天!"));
