...
//屏幕限制
#define LESS_LIST_CX				268									//列表宽度
#define SPLITTER_WIDTH				1									//拆分宽度
...
#define IDC_LAST_GAME_PLAY			312									//游戏记录
...
bool CFrameEncircle::PreDrawEncircleImage(tagEncircleBMP & EncircleImage)
{
	...
	//构造标志
	StationLogo.DrawImage(pDC,10,30);
	EncircleImage.ImageTL.ReleaseDC();
...
CPlatformFrame::~CPlatformFrame()
{
	if(m_pDlgInsurePlaza!=NULL)delete m_pDlgInsurePlaza;
	m_pDlgInsurePlaza=NULL;
...
BOOL CPlatformFrame::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_NAVIGATION+0: // 首页
		{
			...
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/service.html"),szPlatformLink);
       ...
       case IDC_NAVIGATION+1:  // 商城
		{
			...
			//连接地址
		   //m_pHtmlBrowser->Navigate(szNavigation,NULL,NULL,NULL,NULL);
		   ShellExecute(NULL, _T("open"),_T("iexplore.exe"), _T("http://www.85jn.com:808/Pay/PayCardFill.aspx"), NULL, SW_SHOWMAXIMIZED); 
			WebBrowse(szNavigation,true);
			return TRUE;
			//TCHAR szNavigation[256]=TEXT("");
			//_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/Pay/PayIndex.aspx"),szPlatformLink);
	...
	case IDC_NAVIGATION+2:
		{
			...
			//{
			//	m_pDlgInsurePlaza=new CDlgInsurePlaza;
			//}

			//m_pDlgInsurePlaza->ShowInsureView();
			////设置资源
			//AfxSetResourceHandle(GetModuleHandle(NULL));

			if(CGlobalUnits::GetInstance()->m_szInsurePassword[0]!=0)
			{
				OnBnClickedPlatform2();
				return TRUE;
			}
	...
	case IDC_NAVIGATION+4:		//导航按钮
		{
		    ...
		    Information.ShowMessageBox(TEXT("此功能暂未开放!"),MB_ICONERROR,0);
	...
	//构造地址
          TCHAR szNavigation[256]=TEXT("");
          _sntprintf(szNavigation,CountArray(szNavigation),TEXT("zgx/getd.aspx"));

			//打开页面
			CDlgBrowser DlgBrowser;
			DlgBrowser.CreateBrowserControl(TEXT("低保救济"),szNavigation,CSize(1007,703));
			return TRUE;
		//变量定义
			/*ASSERT(CParameterGlobal::GetInstance()!=NULL);
			CParameterGlobal * pParameterGlobal=CParameterGlobal::GetInstance();
			...
			//构造地址
			TCHAR szNavigation[256]=TEXT("");
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/zgx/getd.aspx"),szPlatformLink,(nCommandID-IDC_NAVIGATION)+1);
	...
	//构造地址
           TCHAR szNavigation[256]=TEXT("");
          _sntprintf(szNavigation,CountArray(szNavigation),TEXT("zgx/userd.aspx"));


			//打开页面
			CDlgBrowser DlgBrowser;
			DlgBrowser.CreateBrowserControl(TEXT("签到"),szNavigation,CSize(951,699));
			return TRUE;
		//变量定义
			/*ASSERT(CParameterGlobal::GetInstance()!=NULL);
			CParameterGlobal * pParameterGlobal=CParameterGlobal::GetInstance();
			...
			//构造地址
			TCHAR szNavigation[256]=TEXT("");
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/zgx/userd.aspx"),szPlatformLink,(nCommandID-IDC_NAVIGATION)+1);
...
VOID CPlatformFrame::RectifyControl(INT nWidth, INT nHeight)
{
	...
	if (m_bHideGameList==false)
	{
		...
		INT nLessPos=LESS_LIST_CX,nMaxPos=LESS_LIST_CX;
	...
	CRect rcLastGamePlay;
	m_WndLastGamePlay.GetWindowRect(&rcLastGamePlay);
	...
	DeferWindowPos(hDwp,m_btMax,NULL,nWidth-76,3,0,0,uFlags|SWP_NOSIZE);
	//DeferWindowPos(hDwp,m_btSkin,NULL,nWidth-122,0,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btClose,NULL,nWidth-38,3,0,0,uFlags|SWP_NOSIZE);
	...
	for (INT i=0;i<CountArray(m_btNavigation);i++)
	{
		...
		INT nXExcursion=280+(nSpace+rcNavigation.Width())*i;
		DeferWindowPos(hDwp,m_btNavigation[i],NULL,nXExcursion,26,0,0,uFlags|SWP_NOSIZE);
	}
	...
	DeferWindowPos(hDwp,m_WndUserInfoCtrl,NULL,EncircleInfoFrame.nLBorder,EncircleInfoFrame.nTBorder-17,rcSplitter.left-EncircleInfoFrame.nLBorder,rcUserInfoCtrl.Height(),uFlags);

	DeferWindowPos(hDwp,m_WndLastGamePlay,NULL,EncircleInfoFrame.nLBorder+EncircleInfoList.nLBorder,EncircleInfoFrame.nTBorder+EncircleInfoList.nTBorder+rcUserInfoCtrl.Height()-17,
		rcSplitter.left-EncircleInfoList.nLBorder-EncircleInfoList.nRBorder-EncircleInfoFrame.nLBorder+1,rcLastGamePlay.Height(),uFlags);

	DeferWindowPos(hDwp,m_ServerListView,NULL,EncircleInfoFrame.nLBorder+EncircleInfoList.nLBorder,EncircleInfoFrame.nTBorder+EncircleInfoList.nTBorder+rcUserInfoCtrl.Height()+rcLastGamePlay.Height()-17,
		rcSplitter.left-EncircleInfoList.nLBorder-EncircleInfoList.nRBorder-EncircleInfoFrame.nLBorder+1,nHeight-EncircleInfoList.nTBorder-EncircleInfoList.nBBorder
		-EncircleInfoFrame.nTBorder-rcUserInfoCtrl.Height()-rcLastGamePlay.Height()-EncircleInfoFrame.nBBorder+17,uFlags);
...
CGameServerItem * CPlatformFrame::SearchGameServer(WORD wServerID)
{
	return m_ServerListData.SearchGameServer(wServerID);
}
...
VOID CPlatformFrame::OnClose()
{
	...
	m_WndLastGamePlay.SaveLastPlayGame();
...
BOOL CPlatformFrame::OnEraseBkgnd(CDC * pDC)
{
	...
	//拆分位置
	CRect rcSplitter;
	//rcSplitter.left=LESS_LIST_CX;
	//rcSplitter.right=LESS_LIST_CX+1;
	//rcSplitter.top=0;
	//rcSplitter.bottom=rcClient.Height();
	m_SkinSplitter.GetWindowRect(&rcSplitter);
	...
	if (m_bHideGameList==false)
	{
		...
		rcList.top=FrameEncircleInfo.nTBorder+rcUserInfoCtrl.Height()-17;
...
INT CPlatformFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_SkinSplitter.EnableWindow(FALSE);
	...
	m_WndLastGamePlay.Create(NULL,NULL,WS_VISIBLE|WS_CHILD|WS_CLIPCHILDREN,rcCreate,this,IDC_LAST_GAME_PLAY);
	...
	//导航按钮
	m_btNavigation[0].SetButtonImage(IDB_NAVIGATION_1,hInstance,true,false);
	m_btNavigation[1].SetButtonImage(IDB_NAVIGATION_2,hInstance,true,false);
	m_btNavigation[2].SetButtonImage(IDB_NAVIGATION_3,hInstance,true,false);
	m_btNavigation[3].SetButtonImage(IDB_NAVIGATION_4,hInstance,true,false);
	m_btNavigation[4].SetButtonImage(IDB_NAVIGATION_5,hInstance,true,false);
	m_btNavigation[5].SetButtonImage(IDB_NAVIGATION_6,hInstance,true,false);
	m_btNavigation[6].SetButtonImage(IDB_NAVIGATION_7,hInstance,true,false);

	//拆分设置
	m_SkinSplitter.SetSplitterEvent(this);
	m_SkinSplitter.SetSplitterColor(RGB(21,86,124));
	...
	m_ToolTipCtrl.AddTool(&m_btNavigation[2],TEXT("财富中心"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[3],TEXT("帐号绑定"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[4],TEXT("游戏商城"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[5],TEXT("低保救济"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[6],TEXT("每日签到"));
	...
	m_WndLastGamePlay.LoadLastPlayGame();
...
VOID CPlatformFrame::OnBnClickedPlatform2()
{
	...
	if (bGoldServer==false)
	{
		...
		DlgInsurePlaza.SetInsurePassword(CGlobalUnits::GetInstance()->m_szInsurePassword);
	...
	case EVENT_USER_LOGON:			//登录完成
		{
			...
			CInformation Information(this);
			Information.ShowMessageBox(TEXT(""),TEXT("尊敬的用户,为确保您的账号安全,建议您先绑定后在进行游戏"));
			//用户甜饼
...
void CPlatformFrame::AddLastPlayGame(WORD wServerID)
{
	...
	m_WndLastGamePlay.AddLastPlayGame(wServerID);
