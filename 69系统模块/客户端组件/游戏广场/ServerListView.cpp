...
CServerListView::CServerListView()
{
	...
	m_ImageType.LoadFromResource(AfxGetInstanceHandle(),IDB_SERVER_TYPE);
	m_ImageTypeArrow.LoadImage(AfxGetInstanceHandle(),TEXT("TYPE_ARROW"));
	m_ImageKindLine.LoadImage(AfxGetInstanceHandle(),TEXT("KIND_LINE"));
	m_ImageKindState.LoadImage(AfxGetInstanceHandle(),TEXT("KIND_STATE"));
	m_ImageKindStar.LoadImage(AfxGetInstanceHandle(),TEXT("KIND_STARE"));
	m_ImageServerState.LoadImage(AfxGetInstanceHandle(),TEXT("SERVER_STATE"));

	m_nTypeWidth=m_ImageType.GetWidth()/3;
	m_nTypeHeight=m_ImageType.GetHeight();
	m_nArrowWidth=m_ImageTypeArrow.GetWidth()/2;
	m_nArrowHeight=m_ImageTypeArrow.GetHeight();

	m_KindStateArray.clear();
	
	//创建字体
	m_FontBold.CreateFont(-12,0,0,0,600,0,0,0,134,3,2,ANTIALIASED_QUALITY,2,TEXT("宋体"));
	m_FontUnderLine.CreateFont(-12,0,0,0,400,0,1,0,134,3,2,ANTIALIASED_QUALITY,2,TEXT("宋体"));
...
VOID CServerListView::DrawTreeItem(CDC * pDC, CRect & rcClient, CRect & rcClipBox)
{
	...
	do
	{
		...
		bool bDrawExpanded=(bDrawChildren && (uItemState&TVIS_EXPANDED));
		...
		if (GetItemRect(hItemCurrent,rcItem,TRUE))
		{
			...
			//设置位置
			rcRect.left=0;
			rcRect.top=rcItem.top;
			rcRect.bottom=rcItem.bottom-1;
			rcRect.right=rcClient.Width();

			CRect rcItemClient;
			GetItemRect(hItemCurrent,rcItemClient,FALSE);

			CGameListItem * pGameListTemp=(CGameListItem*)GetItemData(hItemCurrent);
			if(pGameListTemp!=NULL)
			{
				switch(pGameListTemp->GetItemGenre())
				{
				case ItemGenre_Type:
					{
						DrawTreeType(pDC,rcItemClient,bDrawSelected,bDrawExpanded,(m_hItemMouseHover==hItemCurrent),pGameListTemp,hItemCurrent);
						continue;
					}
				case ItemGenre_Kind:
					{
						DrawTreeKind(pDC,rcItem,bDrawSelected,bDrawExpanded,(m_hItemMouseHover==hItemCurrent),pGameListTemp,hItemCurrent);
						continue;;
					}
				//case ItemGenre_Server:
				//	{
				//		DrawTreeServer(pDC,rcItemClient,bDrawSelected,bDrawExpanded,(m_hItemMouseHover==hItemCurrent),pGameListTemp,hItemCurrent);
				//		break;
				//	}
				}
			}
			...
			CGameServerItem* pServerItem=(CGameServerItem*)GetItemData(hItemCurrent);
			if(pServerItem==NULL)
			{
			...
			//绘制状态
			COLORREF crStatus=RGB(255,0,255);
			CString strStatus=TEXT("");
			WORD wStatus=GetLoadInfoOfServer(&pServerItem->m_GameServer);
			if(wStatus!=INVALID_WORD)
			{
				CSize sizeStatus(m_ImageServerState.GetWidth()/5,m_ImageServerState.GetHeight());
				m_ImageServerState.DrawImage(pDC,rcItemClient.left+17,rcItemClient.top+6,sizeStatus.cx,sizeStatus.cy,wStatus*sizeStatus.cx,0,sizeStatus.cx,sizeStatus.cy);

				if(wStatus==0) { crStatus=RGB(152,220,53); strStatus=TEXT("闲"); }
				else if(wStatus==1) { crStatus=RGB(72,210,249); strStatus=TEXT("良"); }
				else if(wStatus==2) { crStatus=RGB(249,170,52); strStatus=TEXT("忙"); }
				else if(wStatus==3) { crStatus=RGB(212,65,27); strStatus=TEXT("挤"); }
				else if(wStatus==4) { crStatus=RGB(168,30,225); strStatus=TEXT("满"); }
				//if(wStatus==0) { crStatus=RGB(108,137,93); strStatus=TEXT("闲"); }
				//else if(wStatus==1) { crStatus=RGB(108,137,93); strStatus=TEXT("良"); }
				//else if(wStatus==2) { crStatus=RGB(234,195,56); strStatus=TEXT("忙"); }
				//else if(wStatus==3) { crStatus=RGB(163,60,43); strStatus=TEXT("挤"); }
				//else if(wStatus==4) { crStatus=RGB(163,60,43); strStatus=TEXT("满"); }
			}
			...
			pDC->SetTextColor(RGB(51,71,81));
			pDC->TextOut(rcItem.left-35,rcItem.top+5,pServerItem->m_GameServer.szServerName);
			
			pDC->SetTextColor(crStatus);
			pDC->TextOut(rcItem.left+75,rcItem.top+5,strStatus);

			pDC->SetTextColor(RGB(131,148,155));
			pDC->SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());
			if(pServerItem->m_GameServer.lMinEnterScore<=10000 || (pServerItem->m_GameServer.lMinEnterScore%10000!=0))
			{
				strStatus.Format(TEXT("%I64d以上可进"),pServerItem->m_GameServer.lMinEnterScore);
			}
			else
			{
				strStatus.Format(TEXT("%I64d萬以上可进"),pServerItem->m_GameServer.lMinEnterScore/10000);
			}
			//strStatus.Format(TEXT("10000以上可进"));
			CRect rcText(rcItem.left+100, rcItem.top+5, rcItem.left+180, rcItem.top+17);//109  198
			pDC->DrawText(strStatus, -1, &rcText, DT_RIGHT|DT_SINGLELINE|DT_VCENTER);
		}
...
//绘制类型
VOID CServerListView::DrawTreeType(CDC * pDC, CRect & rcClient, bool bDrawSelected, bool bDrawChildren, bool bHover,CGameListItem* pListItem,HTREEITEM hItemCurrent)
{
	INT nIndex=0;
	if(bHover) nIndex=2;
	if(bDrawSelected) nIndex=1;
	m_ImageType.StretchBlt(pDC->GetSafeHdc(),rcClient.left,rcClient.top,m_nTypeWidth,ITEM_HEIGHT-1,nIndex*m_nTypeWidth,0,m_nTypeWidth,m_nTypeHeight);

	nIndex=bDrawChildren?1:0;
	m_ImageTypeArrow.DrawImage(pDC,rcClient.left+6,rcClient.top+8,m_nArrowWidth,m_nArrowHeight,m_nArrowWidth*nIndex,0);

	CGameTypeItem* pTypeItem=(CGameTypeItem*)pListItem;
	pDC->SelectObject(m_FontBold);
	pDC->SetBkMode(1);
	pDC->SetTextColor(RGB(13,61,88));
	pDC->TextOut(rcClient.left+21,rcClient.top+7,pTypeItem->m_GameType.szTypeName);
	
}

//绘制游戏
VOID CServerListView::DrawTreeKind(CDC * pDC, CRect & rcItem, bool bDrawSelected, bool bDrawChildren, bool bHove,CGameListItem* pListItemr,HTREEITEM hItemCurrent)
{
	CGameKindItem* pKindItem=(CGameKindItem*)pListItemr;
	...
	//绘制状态修改星星跟图标位置
	for(int i=0; i<m_KindStateArray.size(); i++)
	{
		if(m_KindStateArray[i].wKindID==pKindItem->m_GameKind.wKindID)
		{
			if(m_KindStateArray[i].wState!=0)
			{
				nXPos=rcItem.left+93;
				nYPos=rcItem.top+5;
				m_ImageKindState.DrawImage(pDC,nXPos,nYPos,17,16,(m_KindStateArray[i].wState-1)*17,0);
			}

			nXPos=rcItem.left+145;
			nYPos=rcItem.top+5;
			for(int j=0; j<m_KindStateArray[i].wStar; j++)
			{
				m_ImageKindStar.DrawImage(pDC,nXPos,nYPos);
				nXPos+=11;
			}
...
//绘制房间
VOID CServerListView::DrawTreeServer(CDC * pDC, CRect & rcItem, bool bDrawSelected, bool bDrawChildren, bool bHover,CGameListItem* pListItem,HTREEITEM hItemCurrent)
{
	//绘画经过
	if ((bDrawSelected==false)&&(bHover))
	{
		pDC->FillSolidRect(&rcItem,RGB(204,224,237));
	}
...
VOID CServerListView::OnGameKindFinish(WORD wKindID)
{
	...
	{
		//变量定义
		LPCTSTR pszTitle=TEXT("游戏更新中·尽请期待");
	...
	case ItemGenre_Kind:	//游戏类型
		{
			...
			//构造路径
			TCHAR szConfigPath[MAX_PATH]=TEXT("");
			_sntprintf(szConfigPath,CountArray(szConfigPath),TEXT("%s\\GameOption.INI"),szDirectory);
			
			tagKindState KindState;
			KindState.wKindID=pGameKindItem->m_GameKind.wKindID;
			KindState.wStar=GetPrivateProfileInt(pGameKindItem->m_GameKind.szKindName,TEXT("star"),0,szConfigPath);
			KindState.wState=GetPrivateProfileInt(pGameKindItem->m_GameKind.szKindName,TEXT("state"),0,szConfigPath);

			if(KindState.wStar>5) KindState.wStar=5;
			if(KindState.wState>6)KindState.wState=0;

			m_KindStateArray.push_back(KindState);
		...
		case ItemGenre_Kind:	//游戏种类
			{
				...
				_sntprintf(szRuleLink,CountArray(szRuleLink),TEXT("%s/GameRules/%ld.html"),szPlatformLink,wGameID);
...
//获得房间负载信息
WORD CServerListView::GetLoadInfoOfServer(tagGameServer * pGameServer)
{
	if(pGameServer == NULL) return INVALID_WORD;

	return GetLoadInfoOfServer(pGameServer->dwOnLineCount,pGameServer->dwFullCount);
}

//获得房间负载信息
WORD CServerListView::GetLoadInfoOfServer(DWORD dwOnLineCount, DWORD dwMaxCount)
{
	if(dwMaxCount == 0)dwMaxCount = 2;
