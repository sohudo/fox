...
VOID CGameFrameWnd::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//控制按钮
	DeferWindowPos(hDwp,m_btOption,NULL,nWidth-225,4,0,0,uFlags|SWP_NOSIZE|SWP_HIDEWINDOW);
	DeferWindowPos(hDwp,m_btMuteContrl,NULL,nWidth-205,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btRule,NULL,nWidth-155,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMin,NULL,nWidth-101,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMax,NULL,nWidth-75,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btClose,NULL,nWidth-50,4,0,0,uFlags|SWP_NOSIZE);
...
BOOL CGameFrameWnd::OnEraseBkgnd(CDC * pDC)
{
	...
	if (strTitle.IsEmpty()==false)
	{
		//计算区域
		CRect rcTitle;
		rcTitle.SetRect(5,7,rcClient.Width()-150,FrameEncircleInfo.nTBorder-8);
