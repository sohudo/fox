...
{	
	...
	if (false==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lStorageGold ))
	{
		//构造消息
		CString strMessage;
		if(m_lStorageGold>0)strMessage.Format(TEXT("你输入的欢乐豆值必须在1和%I64d之间"), m_lStorageGold);
		else strMessage.Format(TEXT("你的存储欢乐豆数目为0,不能进行提取操作!"));
	...
	if (true==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lGameGold ))
	{
		//构造消息
		CString strMessage;
		if(m_lGameGold>0)strMessage.Format(TEXT("你输入的欢乐豆值必须在1和%I64d之间"),m_lGameGold );
		else strMessage.Format(TEXT("你的当前欢乐豆数目为0,不能进行存储操作!"));
...
{	
	...
	pDC->DrawText(TEXT("当前欢乐豆："),rcScore,DT_END_ELLIPSIS | DT_WORDBREAK|DT_RIGHT);
	CRect rcBank(100,105,200,120);
	pDC->DrawText(TEXT("保险柜欢乐豆："),rcBank,DT_END_ELLIPSIS | DT_WORDBREAK|DT_RIGHT);
	CRect rect(0,315,nWidth,nHeight);
	CString strInfo;
	strInfo.Format(TEXT("温馨提示：存入欢乐豆免手续费，取出将扣除%ld‰的手续费"), m_wRevenueTake);
