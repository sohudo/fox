...
BOOL CGameFrameUserView::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_pICurrentUserItem!=NULL)
	{
		...
		//else
		{
			...
			pFaceItemControl->DrawFaceBigNormal(pBufferDC,12,12,m_pICurrentUserItem->GetGender());
		}

		//位置定义
		CRect rcNickName(85,nYExcursion+15,rcClient.Width()-12,nYExcursion+27);
		CRect rcExperience(114,nYExcursion+34,rcClient.Width()-12,nYExcursion+46);
		CRect rcUserGameID(114,nYExcursion+53,rcClient.Width()-12,nYExcursion+65);
		...
		_sntprintf(szString,CountArray(szString),TEXT("%s [ %s ]"),m_pICurrentUserItem->GetNickName(),pszUserOrder);
		pBufferDC->DrawText(szString,lstrlen(szString),&rcNickName,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

		//积分
		_sntprintf(szString,CountArray(szString),TEXT("%I64d"),m_pICurrentUserItem->GetUserScore());
