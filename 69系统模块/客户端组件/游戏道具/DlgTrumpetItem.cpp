...
void CDlgTrumpetItem::OnBnClickedTrumpet()
{
	...
	m_strPropertyInfo.Format(TEXT("小喇叭%I64d银子一条。每条消息不得超过%d个字符，消息将在房间内的聊天框内以醒目的方式显示。"),	pPropertyInfo->lPropertyGold,TRUMPET_MAX_CHAR);	
...
void CDlgTrumpetItem::OnBnClickedTyphon()
{
	...
	m_strPropertyInfo.Format(TEXT("大喇叭%I64d银子一条。每条消息不得超过%d个字符，消息将在房间内的聊天框内以醒目的方式显示。"),	pPropertyInfo->lPropertyGold,TRUMPET_MAX_CHAR);
