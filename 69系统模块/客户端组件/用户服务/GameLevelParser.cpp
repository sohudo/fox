...
WORD CGameLevelParser::GetDefaultLevelNum(SCORE lScore)
{

	//等级积分
	static SCORE lLevelScore[]=
	{	
		0L,1000L,10000L,100000L,400000L,800000L,1600000L,3200000L,6400000L,12800000L,51200000L,100000000L
	};	

	//等级名称
	static LPCTSTR pszLevelName[]=	
	{		
		TEXT("待业"),TEXT("正式工"),TEXT("主管"),TEXT("经理"),TEXT("总裁"),TEXT("董事"),TEXT("董事长"),TEXT("千王"),TEXT("至尊"),TEXT("无敌"),TEXT("财神"),TEXT("首富")
	};
	...
	{		
		if (lScore>=lLevelScore[CountArray(lLevelScore)-i-1]) return CountArray(lLevelScore)-i-1;	
	}	
...
//等级描述
WORD CGameLevelParser::GetLevelNum(SCORE lScore)
{
	//状态效验
	ASSERT(m_LevelDescribeActive.GetCount()>0L);
	if (m_LevelDescribeActive.GetCount()==0) return GetDefaultLevelNum(lScore);
	...
	{
		tagLevelDescribe * pLevelDescribe=m_LevelDescribeActive[i];
		if ((lScore>=pLevelDescribe->lLevelScore)||(i==(wItemCount-1))) return i;
	}

	return 0;
}
//等级描述
WORD CGameLevelParser::GetLevelNum(IClientUserItem * pIClientUserItem)
{
	//状态效验
	ASSERT(m_LevelDescribeActive.GetCount()>0L);
	if (m_LevelDescribeActive.GetCount()==0) return GetDefaultLevelNum(pIClientUserItem->GetUserScore());

	//变量定义
	SCORE lUserScore=pIClientUserItem->GetUserScore();
	WORD nItemCount=m_LevelDescribeActive.GetCount();

	//等级搜索
	for (WORD i=0;i<nItemCount;i++)
	{
		tagLevelDescribe * pLevelDescribe=m_LevelDescribeActive[nItemCount-i-1];
		if ((lUserScore>=pLevelDescribe->lLevelScore)||(i==(nItemCount-1))) 
			return nItemCount-i-1;
	}
