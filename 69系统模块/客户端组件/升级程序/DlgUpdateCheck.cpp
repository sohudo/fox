...
BOOL CDlgUpdateCheck::OnInitDialog()
{
	...
	//构造地址
	m_pszDownLoadFile.Format(TEXT("%s/Update.txt"),szDownloadLink);
...
//	ImageStatusPoint.LoadImage(AfxGetInstanceHandle(),TEXT("PROGRESS_POINT"));
	...
	//绘画进度
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,40,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,40,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	ImageStatusPoint.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2+nXPoint-SizeStatusPoint.cx/2,40+(SizeStatusLine.cy-SizeStatusPoint.cy)/2);
