...
//窗口标识
#define IDC_DOWNLOAD_MANAGER        300                                 //窗口标识 
...
//执行下载
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszNetAddress,LPCTSTR pszFileName,bool bBreakpointDownLoad)
{
	//效验状态
	...
	//发送消息
	if(m_pDownLoadManager!=NULL) m_pDownLoadManager->PostMessage(WM_ITEM_DOWNLOAD_UPDATE,0,0);

	return true;
}

VOID CDownLoadSink::PerformInstall()
{
	//下载文件
	...
	GetWorkDirectory(szWorkDirectory,CountArray(szWorkDirectory));
	...
	//发送消息
	if(m_pDownLoadManager!=NULL) m_pDownLoadManager->PostMessage(WM_ITEM_DOWNLOAD_Finish,(WPARAM)pDownLoadResult);

	return;
}

//重置变量
VOID CDownLoadSink::ResetDownLoadSink()
{
	//下载速度
	m_dwDownSpeed=0L;
	m_dwLastCalcSize=0L;
...
bool CDownLoadSink::OnDataStream(const VOID * pcbMailData, WORD wStreamSize)
{
	...
	//发送消息
	if(m_pDownLoadManager!=NULL) m_pDownLoadManager->SendMessage(WM_ITEM_DOWNLOAD_UPDATE,m_dwDownLoadSize==m_dwTotalFileSize);
	
	return true;
}

//下载信息
bool CDownLoadSink::OnDataInformation(bool bPartialConent,DWORD dwTotalFileSize, LPCTSTR pszEntityTag, LPCTSTR pszLocation)
{
	//设置变量
	...
	//发送消息
	if(m_pDownLoadManager!=NULL) m_pDownLoadManager->SendMessage(WM_ITEM_DOWNLOAD_UPDATE);
...
//加载文件
bool CDownLoadSink::LoadDownFile(LPCTSTR pszLocation)
{
	//效验状态
...
CDownLoadManager::CDownLoadManager()
{
	...
	ZeroMemory(m_szDownLoadAddress,sizeof(m_szDownLoadAddress));
...
bool CDownLoadManager::ReadDownLoadInfo()
{
	...
	GetWorkDirectory(szTargetFilePath,CountArray(szTargetFilePath));
...
bool CDownLoadManager::StartDownLoadTask()
{
	...
	if(m_pDownLoadSink==NULL)
	{
		...
		{
			//创建指针
			m_pDownLoadSink = new CDownLoadSink();
...
VOID CDownLoadManager::PerformMoveFiles()
{
	...
	while(m_SourcePathArray.GetCount()>0)
	{
		...
		//移动文件
		if(MoveFileWithProgress(szSourceFilePath,szTargetFilePath,NULL,NULL,MOVEFILE_WRITE_THROUGH|MOVEFILE_REPLACE_EXISTING))
		{
			//删除文件
		...
		else
		{
			...
			//修改名字
			MoveFile( szTargetFilePath,szTargetNameEx );
...
VOID CGameUpdateFrame::OnEventDownLoadEnd()
{
	...
	_sntprintf(szMainSetupPath,CountArray(szMainSetupPath),TEXT("%s/%s"),szMouduleFileName,szPlazaName);
...
INT CGameUpdateFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	UpdateFrameBack.LoadImage(AfxGetInstanceHandle(),TEXT("UPDATE_FRAME_BACK"));
...
VOID CGameUpdateFrame::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	//变量定义
	CPngImage ImageProgressLine,ImageProgressPoint;

	//加载图像
	ImageProgressLine.LoadImage(AfxGetInstanceHandle(),TEXT("PROGRESS_LINE"));
	ImageProgressPoint.LoadImage(AfxGetInstanceHandle(),TEXT("PROGRESS_POINT"));

	//变量定义
	CRect rcClient(0,0,0,0);
	GetClientRect(rcClient);
	CSize ProgressLineSize(ImageProgressLine.GetWidth(),ImageProgressLine.GetHeight()/2);
	CSize ProgressPointSize(ImageProgressPoint.GetWidth(),ImageProgressPoint.GetHeight());

	//变量定义
	int nProgressLeft = (rcClient.Width()-ProgressLineSize.cx)/2;

	//进度条背景
	ImageProgressLine.DrawImage(pDC,nProgressLeft,42,ProgressLineSize.cx,ProgressLineSize.cy,0,0);
	ImageProgressLine.DrawImage(pDC,nProgressLeft,82,ProgressLineSize.cx,ProgressLineSize.cy,0,0);
	...
	if(m_pDownLoadManager!=NULL && m_pDownLoadManager->m_pDownLoadSink!=NULL)
	{
		...
		//绘制进度
		ImageProgressLine.DrawImage(pDC,nProgressLeft,42,dwTotalProgress,ProgressLineSize.cy,0,ProgressLineSize.cy);
		ImageProgressLine.DrawImage(pDC,nProgressLeft,82,dwCurrProgress,ProgressLineSize.cy,0,ProgressLineSize.cy);

		//绘制点
		ImageProgressPoint.DrawImage(pDC,nProgressLeft+dwTotalProgress-ProgressPointSize.cx/2,38);
		ImageProgressPoint.DrawImage(pDC,nProgressLeft+dwCurrProgress-ProgressPointSize.cx/2,78);	
		...
		//绘制信息
		pDC->TextOut(nProgressLeft,62,szTotalProgress);
		pDC->TextOut(nProgressLeft,108,szCurrProgress);
		pDC->TextOut(330,108,szNetSpeed);
