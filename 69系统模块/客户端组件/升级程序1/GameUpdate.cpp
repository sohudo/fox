...
BOOL CGameUpdateApp::InitInstance()
{
	...
	//格式字串
	_sntprintf(szSourceName,CountArray(szSourceName),TEXT("%s/DownLoad/%s"),szMouduleFileName,szUpdateName);
	_sntprintf(szTargetName,CountArray(szTargetName),TEXT("%s/%s"),szMouduleFileName,szUpdateName);
	...
	DlgUpdateCheck.SetWindowDescribe(TEXT("正在检查更新,请稍后..."));
