...
bool CUpdateCheckSink::CloseDownLoad()
{
	...
	TCHAR szGameUpdate[MAX_PATH]=TEXT("");
	_sntprintf(szGameUpdate,CountArray(szGameUpdate),TEXT("%s\\%s"),szMouduleFileName,szUpdataName);

    

	WritePrivateProfileString(TEXT("Update"), TEXT("XmlFileName"), szPlazaUpdateName, szGameUpdate);
	WritePrivateProfileString(TEXT("Update"), TEXT("State"), TEXT("0"), szGameUpdate);
...
bool CUpdateCheckSink::PerformCheckSum()
{
	...
	{
		//计算大小
		dwUnCompressSize=m_dwTotalFileSize*4;
		...
		//数据解压缩
		dwUnCompressSize=UnCompressData(m_pFileData,m_dwTotalFileSize,pUnCompressData,dwUnCompressSize);
...
BOOL CDlgUpdateCheck::OnInitDialog()
{
	...
	TCHAR szXmlFileName[MAX_PATH]=TEXT("");
	GetPrivateProfileString(TEXT("Update"),TEXT("XmlFileName"),TEXT(""),szXmlFileName, 50, szGameUpdate);
    
	//构造地址
	m_pszDownLoadFile.Format(TEXT("%s/DownLoad/Client/%s"),szDownloadLink,szXmlFileName);
...
{
	//结束模态
	EndDialog(bNeedUpdate==TRUE?IDCANCEL:IDOK);
}
...
BOOL CDlgUpdateCheck::OnEraseBkgnd(CDC * pDC)
{
	...
	ImageStatusPoint.LoadImage(AfxGetInstanceHandle(),TEXT("PROGRESS_POINT"));
	...
	//绘画进度
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,42,SizeStatusLine.cx,SizeStatusLine.cy,0,0);
	ImageStatusLine.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2,42,SizeStatusLine.cx*m_uImageIndex/100L,SizeStatusLine.cy,0,SizeStatusLine.cy);

	//绘画进度
	INT nXPoint=SizeStatusLine.cx*m_uImageIndex/100L;
	ImageStatusPoint.DrawImage(pBufferDC,(rcClient.Width()-SizeStatusLine.cx)/2+nXPoint-SizeStatusPoint.cx/2,42+(SizeStatusLine.cy-SizeStatusPoint.cy)/2);

	//文字位置
	CRect rcDescribe;
	rcDescribe.SetRect(95,16,95+184,16+18);
	...
	//绘画文字
	pBufferDC->SetTextColor(RGB(241,204,129));
