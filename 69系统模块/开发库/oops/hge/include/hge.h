...
enum hgeBoolState
{
	HGE_WINDOWED		= 1,    // bool		运行在窗口?		(default: false)
	HGE_ZBUFFER			= 2,    // bool		使用Z -缓冲?	(default: false)
	HGE_TEXTUREFILTER	= 3,    // bool		纹理过滤?		(default: true)
	
	HGE_USESOUND		= 4,    // bool		使用BASS声音?	(default: true)
	
	HGE_DONTSUSPEND		= 5,	// bool		焦点丢失：暂停?	(default: false)
	HGE_HIDEMOUSE		= 6,	// bool		隐藏系统光标?	(default: true)

	HGE_SHOWSPLASH		= 7,	// bool		显示启动?	(default: true)
...
enum hgeFuncState
{
	HGE_FRAMEFUNC		= 8,    // bool*()	框架函数		(default: NULL) (你必须设置此)
	HGE_RENDERFUNC		= 9,    // bool*()	渲染函数		(default: NULL)
	HGE_FOCUSLOSTFUNC	= 10,   // bool*()	焦点失去函数	(default: NULL)
	HGE_FOCUSGAINFUNC	= 11,   // bool*()	焦点获得函数	(default: NULL)
	HGE_GFXRESTOREFUNC	= 12,   // bool*()	exit function	(default: NULL)
	HGE_EXITFUNC		= 13,   // bool*()	退出函数		(default: NULL)
...
enum hgeHwndState
{
	HGE_HWND			= 15,	// int		窗口句柄：只读
	HGE_HWNDPARENT		= 16,	// int		父窗口句柄	(default: 0)
...
enum hgeIntState
{
	HGE_SCREENWIDTH		= 17,   // int		屏幕宽度		(default: 800)
	HGE_SCREENHEIGHT	= 18,   // int		屏幕高度		(default: 600)
	HGE_SCREENBPP		= 19,   // int		屏幕位深度		(default: 32) (desktop bpp in windowed mode)
	
	HGE_SAMPLERATE		= 20,   // int		采样率			(default: 44100)
	HGE_FXVOLUME		= 21,   // int		global fx volume	(default: 100)
	HGE_MUSVOLUME		= 22,   // int		全局音乐音量	(default: 100)
	HGE_STREAMVOLUME	= 23,   // int		global music volume	(default: 100)
	
	HGE_FPS				= 24,	// int		固定FPS			(default: HGEFPS_UNLIMITED)

	HGE_POWERSTATUS		= 25,   // int		电源状态
...
enum hgeStringState
{
	HGE_ICON			= 26,   // char*	图标资源		(default: NULL)
	HGE_TITLE			= 27,   // char*	窗口标题		(default: "HGE")
	
	HGE_INIFILE			= 28,   // char*	ini文件			(default: NULL) (没有文件的含义)
	HGE_LOGFILE			= 29,   // char*	日志文件		(default: NULL) (没有文件的含义)
